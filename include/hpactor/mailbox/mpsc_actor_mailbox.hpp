...
template <typename T> class MPSCActorMailbox {
  public:
    ...
    auto reserve_result = reservation_.try_reserve(meta.estimated_bytes,
...
void enqueue_reserved(T* node, const MailboxEnvelopeMeta& meta,
                      uint8_t lane_idx = 0, bool suppress_wakeup = false) noexcept {
    ...
    if (depth > 1024) [[unlikely]] {
        HPACTOR_LOG_WARNING(log::LogCategory::kMailbox, actor_id_,
...
void drain_overflow() noexcept {
    ...
    while (config_.overflow_policy == OverflowPolicy::SpillToOverflowQueue) {
        ...
        enqueue_reserved(
            new (mem::allocate(mem::RegionType::kMessage, sizeof(T), actor_id_))
...
void update_pressure_state(bool hard_failure = false) noexcept {
    pressure_state_.update(pressure_ratio(), hard_failure, config_.high_watermark,
