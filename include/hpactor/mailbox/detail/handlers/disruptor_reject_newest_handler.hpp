...
/// counter and returns \c Rejected with a backpressure reason.
///
...
class DisruptorRejectNewestHandler
  ...
  public:
    EnqueueResult handle(DisruptorOverflowContext<EnvelopeType>& ctx,
        ...
        return build_result(ctx, EnqueueResultCode::Rejected,
                            reason == adt::ReservationResult::ByteCapacity
  ...
  private:
    ...
    build_result(DisruptorOverflowContext<EnvelopeType>& ctx,
                 EnqueueResultCode code, BackpressureReason reason) {
        EnqueueResult r;
        r.code = code;
