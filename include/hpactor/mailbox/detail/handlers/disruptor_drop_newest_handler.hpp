...
///
/// Silently discards the incoming message and returns \c DroppedNewest.
/// The envelope is not stored anywhere; the caller simply drops it.
///
/// \tparam EnvelopeType The disruptor message envelope type.
template <typename EnvelopeType>
class DisruptorDropNewestHandler : public IDisruptorOverflowHandler<EnvelopeType> {
  public:
