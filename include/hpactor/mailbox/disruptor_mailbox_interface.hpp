...
struct DisruptorDeliveryObservation {
    ActorId actor;
    uint64_t message_id;
...
struct DisruptorDeliveryFailure {
    ActorId actor;
    ActorAddress sender;
...
/// production paths.
struct DisruptorMailboxDelivery {
    void* context{nullptr};

    DeliveryPreflightResult (*preflight)(void*, const ActorAddress&,
                                         const DisruptorEnvelopeMeta&) noexcept {
        nullptr};

    void (*record_accepted)(void*, ActorId,
                            const DisruptorDeliveryObservation&) noexcept {nullptr};

    void (*record_rejected)(void*, ActorId,
                            const DisruptorDeliveryFailure&) noexcept {nullptr};
};
...
struct DisruptorMailboxExecution {
    void* context{nullptr};
...
struct DisruptorMailboxLifecycle {
    void* context{nullptr};
...
struct DisruptorMailboxHandle {
    /// Retains the shared \c DisruptorActorMailboxCore so it outlives
    /// the actor and can reject surviving-reference sends.
    std::shared_ptr<void> lifetime;

    DisruptorMailboxDelivery delivery;
    DisruptorMailboxControl control;
    DisruptorMailboxExecution execution;
    DisruptorMailboxLifecycle lifecycle;
