...
std::string PrettyFormatter::horizontal_rule(size_t width) {
    ...
    for (size_t i = 0; i < width; ++i) result += kHDash;
    return result;
}
...
void PrettyFormatter::key_value(const std::map<std::string, std::string>& pairs) {
    size_t max_key = 0;
    for (const auto& kv : pairs) max_key = std::max(max_key, kv.first.size());
    for (const auto& kv : pairs)
        buffer_ += "  " + cyan(pad_right(kv.first, max_key + 2)) + green(kv.second) + "\n";
}

void PrettyFormatter::tree(const TreeNode& root) {
    auto print_node = [this](auto& self, const TreeNode& node, int depth, bool last) -> void {
        std::string indent(static_cast<size_t>(depth) * 2, ' ');
        if (depth > 0)
            buffer_ += indent + std::string(last ? kLast : kBranch) + " ";
        buffer_ += bold(node.name);
        if (!node.description.empty()) buffer_ += "  " + dim(node.description);
        buffer_ += "\n";
...
std::string PrettyFormatter::finalize() { return std::move(buffer_); }
