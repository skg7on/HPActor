...
ActorSpawner::adopt_unpublished(std::shared_ptr<AbstractActor> actor,
    ...
    if (!actor) {
        return result<ActorSpawnLease>::make(
            error(errors::invalid_argument, "null actor"));
    }
    ...
    if (spec.reserved_id.has_value()) {
        if (!actor->is_system_actor()) {
            return result<ActorSpawnLease>::make(error(
                errors::invalid_argument, "reserved id requires system actor"));
        }
        if (!is_reserved_id(*spec.reserved_id)) {
            return result<ActorSpawnLease>::make(
                error(errors::invalid_argument, "reserved id outside system range"));
        }
        id = *spec.reserved_id;
    } else {
        id = deps_.directory.allocate_id();
    ...
    if (entry.mailbox_kind == mailbox::MailboxKind::Disruptor) {
        if (!entry.fixed_mailbox.valid()) {
            ...
            if (!binding.valid()) {
                return result<ActorSpawnLease>::make(
                    error(errors::invalid_argument, "fixed mailbox binding is invalid"));
            }
            entry.fixed_mailbox = binding;
        }
    } else {
    ...
    auto status = deps_.directory.publish(std::move(entry), std::nullopt);
    if (status != ActorDirectory::PublishStatus::Published) {
        return result<ActorSpawnLease>::make(
            error(errors::invalid_argument, "duplicate actor id"));
    }
    ...
    return result<ActorSpawnLease>::make(
        ActorSpawnLease(Actor(actor), id, &deps_.directory, &deps_.scheduler));
}
