...
void RegistrarServer::handle_accept(int client_fd) {
    ...
    // For now, we'll read header first, then payload
    // In production, this would be non-blocking with EventLoop integration
    ...
    while (running_.load()) {
        ...
        (void)type; // Will be used in full message handling
        ...
        if (total_read == length) {
            handle_tcp_message(client_fd, payload);
        }
    }
...
void RegistrarServer::handle_tcp_message(int client_fd, const bytes& data) {
    // This is a stub implementation - full message handling will be in later tasks
    // The actual Register, Heartbeat, etc. message parsing will be added as needed
    (void)client_fd;
    (void)data;
}
