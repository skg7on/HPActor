...
// ---------------------------------------------------------------------------
// Raw socket creation + async connect (static helpers)
// ---------------------------------------------------------------------------

/// Complete a non-blocking connect, returning the fd on success.
/// Shared by connect_raw_tcp / connect_raw_uds and the member connect()
/// methods (which wrap the result in WireFrameConnection).
static bool
complete_raw_connect(EventLoop& loop, int fd, std::chrono::milliseconds timeout) {
    auto done = std::make_shared<bool>(false);
    auto success = std::make_shared<bool>(false);

    loop.add_fd(fd, EventLoop::Event::Write);
...
int TcpTransport::connect_raw_tcp(const std::string& host, uint16_t port,
                                  EventLoop& loop,
    ...
    int ret =
        ::connect(fd, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr));
    if (ret == 0)
        return fd; // connected immediately (e.g. loopback)
    if (errno != EINPROGRESS) {
        ::close(fd);
        return -1;
    }

    if (!complete_raw_connect(loop, fd, timeout)) {
        ::close(fd);
        return -1;
    }
    return fd;
...
int TcpTransport::connect_raw_uds(const std::string& path, EventLoop& loop,
                                  std::chrono::milliseconds timeout) {
    int fd = ::socket(AF_UNIX, SOCK_STREAM, 0);
    if (fd < 0)
        return -1;
