...
//
// Shared between thread_loop(), backoff(), and idle().
//
...
void WorkerThread::thread_loop() {
    ...
    while (!stop_requested_.load(std::memory_order_acquire) &&
        ...
        // Threshold kPollThreshold is platform-specific (see top of file).
        if (idle(item)) {
            diag_work_found_.fetch_add(1, std::memory_order_relaxed);
            reset_backoff();
            if (processor_) {
...
void WorkerThread::backoff() {
    ...
    // CV after kBackoffSleepIters idle iterations.  The 50µs/100µs entries
    // are defence-in-depth for edge cases (e.g. standalone workers without
    // CV access, or delayed CV escalation).
    static constexpr uint32_t kSleepUs[] = {10, 20, 50, 100};
    uint32_t idx = c - kBackoffYieldIters;
