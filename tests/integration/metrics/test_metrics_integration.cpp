...
TEST(MetricsIntegrationTest, EndToEndPipeline) {
    MpscRingBuffer<MetricEvent> ring_buf;
    MetricRegistry registry;

    // Register families manually (same as Aggregator would do)
    auto& depth_fam = registry.register_family("hpactor_mailbox_depth",
                                               "Current mailbox queue depth.",
                                               MetricType::kGauge);
    auto& msg_fam = registry.register_family("hpactor_mailbox_messages_total",
                                             "Total messages enqueued.",
                                             MetricType::kCounter);
    auto& latency_fam = registry.register_family("hpactor_message_processing_"
                                                 "seconds",
    ...
    ring_buf.drain([&](const MetricEvent& e) {
        switch (e.event_type) {
            case MetricEventType::kMailboxEnqueue: {
                if (!depth_g)
                    depth_g =
                        &registry.get_or_create<GaugeValue>(depth_fam, actor_ls);
                depth_g->value.fetch_add(1, std::memory_order_relaxed);
                if (!msg_c)
                    msg_c =
                        &registry.get_or_create<CounterValue>(msg_fam, actor_ls);
            ...
            case MetricEventType::kMessageProcessed: {
                if (!lat_h)
                    lat_h = &registry.get_or_create<HistogramValue>(latency_fam,
                                                                    actor_ls);
    ...
    // Verify aggregated values
    {
        ...
        for (const auto& fs : snap.families) {
            if (fs.name == "hpactor_mailbox_depth") {
                ASSERT_FALSE(fs.gauges.empty());
                EXPECT_EQ(fs.gauges[0].second, 2); // 3 enqueue - 1 dequeue
            }
            if (fs.name == "hpactor_mailbox_messages_total") {
                ASSERT_FALSE(fs.counters.empty());
                EXPECT_EQ(fs.counters[0].second, 3);
            }
            if (fs.name == "hpactor_message_processing_seconds") {
                ASSERT_FALSE(fs.histograms.empty());
    ...
    EXPECT_NE(text.find("# HELP hpactor_mailbox_depth"), std::string::npos);
    EXPECT_NE(text.find("# TYPE hpactor_mailbox_depth gauge"), std::string::npos);
    EXPECT_NE(text.find("# HELP hpactor_mailbox_messages_total"), std::string::npos);
    EXPECT_NE(text.find("# TYPE hpactor_mailbox_messages_total counter"),
              std::string::npos);
    EXPECT_NE(text.find("# HELP hpactor_message_processing_seconds"),
              std::string::npos);
    EXPECT_NE(text.find("# TYPE hpactor_message_processing_seconds histogram"),
              std::string::npos);
...
TEST(MetricsIntegrationTest, MultiProducerRingBuffer) {
