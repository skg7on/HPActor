...
template <typename Derived> class object_api : public pyobject_tag {
  ...
  public:
    ...
    object must be a collection which supports the iteration protocol.
    \endrst */
    ...
    `object` subclass causes a corresponding call to ``__getitem__``. Assigning
    a `handle` or `object` subclass causes a call to ``__setitem__``.
    \endrst */
    item_accessor operator[](handle key) const;
    /// See above (the only difference is that the key's reference is stolen)
    item_accessor operator[](object&& key) const;
    /// See above (the only difference is that the key is provided as a string
    /// literal)
    item_accessor operator[](const char* key) const;

    /** \rst
        Return an internal functor to access the object's attributes. Casting
    the returned ``detail::obj_attr_accessor`` instance to a `handle` or
    `object` subclass causes a corresponding call to ``getattr``. Assigning a
    `handle` or `object` subclass causes a call to ``setattr``.
    \endrst */
    ...
    ``tuple`` or ``list`` for a function call. Applying another * to the result
    yields
    ...
    /** \rst
        ...
        may need to be converted back into a Python object using
    `handle::cast()`.
...
#ifdef PYPY_VERSION
        // PyPy uses the top few bits for REFCNT_FROM_PYPY &
        // REFCNT_FROM_PYPY_LIGHT Following pybind11 2.12.1 and older behavior
    ...
    //     "Call py::type::handle_of(h) or py::type::of(h) instead of
    //     h.get_type()")
...
template <typename T>
using is_pyobj_ptr_or_nullptr_t =
    detail::any_of<std::is_same<T, PyObject*>, std::is_same<T, PyObject* const>,
...
class handle : public detail::object_api<handle> {
  public:
    ...
    /// Not using ``handle(PyObject *ptr)`` to avoid implicit conversion from
    /// ``0``.
    template <typename T,
    ...
    template <typename T,
              detail::enable_if_t<
                  detail::all_of<detail::none_of<std::is_base_of<handle, T>, detail::is_pyobj_ptr_or_nullptr_t<T>>,
    ...
    is preferable to use the `object` class which derives from `handle` and
    calls this function automatically. Returns a reference to itself.
    \endrst */
    ...
    `cast_error` will be throw upon failure.
    \endrst */
  ...
  private:
    ...
    void throw_gilstate_error(const std::string& function_name) const {
        ...
        if (Py_TYPE(m_ptr)->tp_name != nullptr) {
            fprintf(stderr, " The failing %s call was triggered on a %s object.",
                    function_name.c_str(), Py_TYPE(m_ptr)->tp_name);
        }
        fprintf(stderr, "\n");
        fflush(stderr);
...
class object : public handle {
  public:
    ...
    object(const object& o) : handle(o) {
        inc_ref();
    }
...
#define PYBIND11_INPLACE_OP(iop)                                               \
    object iop(object_api const& other) {                                      \
        return operator=(handle::iop(other));                                  \
    }
...
#undef PYBIND11_INPLACE_OP
  ...
  public:
    ...
    object(handle h, borrowed_t) : handle(h) {
        inc_ref();
    }
...
struct error_fetch_and_normalize {
    ...
    //     from Sep 2016) and safest. Normalization could be deferred, but this
    //     could mask errors elsewhere, the performance gain is very minor in
    //     typical situations (usually the dominant bottleneck is EH unwinding),
    //     and the implementation here would be more complex.
    // Starting with Python 3.12, PyErr_Fetch() normalizes exceptions
    // immediately. Any errors during normalization are tracked under __notes__.
...
#else
        // PyErr_NormalizeException() may change the exception type if there are
        // cascading failures. This can potentially be extremely confusing.
...
#    if defined(PYPY_VERSION_NUM) && PYPY_VERSION_NUM < 0x07030a00
        // This behavior runs the risk of masking errors in the error handling,
        // but avoids a conflict with PyPy, which relies on the normalization
        // here to change OSError to FileNotFoundError
...
#    endif
    ...
    std::string format_value_and_trace() const {
        ...
        if (m_value) {
            auto value_str =
                reinterpret_steal<object>(PyObject_Str(m_value.ptr()));
            ...
            if (!value_str) {
                ...
                // Not using `value_str.cast<std::string>()`, to not potentially
                // throw a secondary error_already_set that will then result in
                // process termination (#4288).
                auto value_bytes =
                    reinterpret_steal<object>(PyUnicode_AsEncodedString(
                ...
                if (!value_bytes) {
                    ...
                    char* buffer = nullptr;
                    Py_ssize_t length = 0;
                    if (PyBytes_AsStringAndSize(value_bytes.ptr(), &buffer,
                                                &length) == -1) {
                        message_error_string = detail::error_string();
                        result = message_unavailable_exc;
                    } else {
            ...
            auto notes = reinterpret_steal<object>(
                PyObject_GetAttrString(m_value.ptr(), "__notes__"));
            if (!notes) {
                ...
                if (len_notes < 0) {
                    ...
                    for (ssize_t i = 0; i < len_notes; i++) {
                        ...
                        if (!note_bytes) {
                            ...
                            char* buffer = nullptr;
                            Py_ssize_t length = 0;
                            if (PyBytes_AsStringAndSize(note_bytes.ptr(),
                                                        &buffer, &length) == -1) {
...
class PYBIND11_EXPORT_EXCEPTION error_already_set : public std::exception {
  public:
    ...
    error_already_set()
        : m_fetched_error{new detail::error_fetch_and_normalize(
                              "pybind11::error_already_set"),
                          m_fetched_error_deleter} {}

    /// The what() result is built lazily on demand.
    /// WARNING: This member function needs to acquire the Python GIL. This can
    /// lead to
    ///          crashes (undefined behavior) if the Python interpreter is
    ///          finalizing.
    const char* what() const noexcept override;

    /// Restores the currently-held Python error (which will clear the Python
    /// error indicator first if already set). NOTE: This member function will
    /// always restore the normalized exception, which may or may
    ///       not be the original Python exception.
    /// WARNING: The GIL must be held when this member function is called!
    void restore() {
        m_fetched_error->restore();
    }

    /// If it is impossible to raise the currently-held error, such as in a
    /// destructor, we can write it out using Python's unraisable hook
    /// (`sys.unraisablehook`). The error context should be some object whose
    /// `repr()` helps identify the location of the error. Python already knows
    /// the type and value of the error, so there is no need to repeat that.
    void discard_as_unraisable(object err_context) {
        restore();
        PyErr_WriteUnraisable(err_context.ptr());
    }
    /// An alternate version of `discard_as_unraisable()`, where a string
    /// provides information on the location of the error. For example,
    /// `__func__` could be helpful. WARNING: The GIL must be held when this
    /// member function is called!
    void discard_as_unraisable(const char* err_context) {
        discard_as_unraisable(
            reinterpret_steal<object>(PYBIND11_FROM_STRING(err_context)));
    }
    ...
    bool matches(handle exc) const {
        return m_fetched_error->matches(exc);
    }
...
/// Replaces the current Python error indicator with the chosen error,
/// performing a 'raise from' to indicate that the chosen error was caused by
/// the original error.
...
/// Sets the current Python error indicator with the chosen error, performing a
/// 'raise from' from the error contained in error_already_set to indicate that
/// the chosen error was caused by the original error.
inline void
raise_from(error_already_set& err, PyObject* type, const char* message) {
    err.restore();
...
PYBIND11_WARNING_DISABLE_MSVC(4522) // warning C4522: multiple assignment
                                    // operators specified
...
class accessor : public object_api<accessor<Policy>> {
  ...
  private:
    static object ensure_object(object&& o) {
        return std::move(o);
    }
...
struct tuple_item {
    ...
    static void set(handle obj, const IdxType& index, handle val) {
        ...
        if (PyTuple_SetItem(obj.ptr(), ssize_t_cast(index), val.inc_ref().ptr()) !=
            0) {
            throw error_already_set();
        }
    }
...
class sequence_slow_readwrite {
  protected:
    ...
    reference dereference() const {
        return {obj, static_cast<size_t>(index)};
    }
    void increment() {
        ++index;
    }
...
class dict_readonly {
  protected:
    ...
    dict_readonly(handle obj, ssize_t pos) : obj(obj), pos(pos) {
        increment();
    }
...
inline bool PyEllipsis_Check(PyObject* o) {
...
#define PYBIND11_OBJECT_COMMON(Name, Parent, CheckFun)                               \
  public:                                                                            \
    ...
    bool check() const {                                                             \
        return m_ptr != nullptr && (CheckFun(m_ptr) != 0);                           \
    }                                                                                \
    ...
    template <typename Policy_> /* NOLINTNEXTLINE(google-explicit-constructor)       \
                                 */                                                  \
                                Name(const ::pybind11::detail::accessor<Policy_>& a) \
        : Name(object(a)) {}
...
class type : public object {
  public:
    ...
    static handle handle_of(handle h) {
        return handle((PyObject*)Py_TYPE(h.ptr()));
    }

    /// Return a type object from a handle or an object
    static type of(handle h) {
        return type(type::handle_of(h), borrowed_t{});
    }
    ...
    /// standard types, like int, float. etc. yet.
    /// See https://github.com/pybind/pybind11/issues/2486
    template <typename T> static type of() {
...
#ifdef PYBIND11_HAS_STRING_VIEW
    // enable_if is needed to avoid "ambiguous conversion" errors (see PR
    // #3521).
    template <typename T,
              detail::enable_if_t<std::is_same<T, std::string_view>::value, int> = 0>
    // NOLINTNEXTLINE(google-explicit-constructor)
    str(T s) : str(s.data(), s.size()) {}

#    ifdef PYBIND11_HAS_U8STRING
    // reinterpret_cast here is safe (C++20 guarantees char8_t has the same
    // size/alignment as char) NOLINTNEXTLINE(google-explicit-constructor)
    str(std::u8string_view s)
...
class bytes : public object {
  public:
    ...
    bytes(const char* c, const SzType& n)
        : object(PYBIND11_BYTES_FROM_STRING_AND_SIZE(c, ssize_t_cast(n)),
                 stolen_t{}) {
        if (!m_ptr) {
            pybind11_fail("Could not allocate bytes object!");
        }
    ...
    // Obtain a string view that views the current `bytes` buffer value.  Note
    // that this is only valid so long as the `bytes` instance remains alive and
    // so generally should not outlive the lifetime of the `bytes` instance.
    // NOLINTNEXTLINE(google-explicit-constructor)
...
// are included in the doxygen group; close here and reopen after as a
// workaround
/// @} pytypes
...
inline str::str(const bytes& b) {
    ...
    auto obj =
        reinterpret_steal<object>(PyUnicode_FromStringAndSize(buffer, length));
    if (!obj) {
        if (PyErr_Occurred()) {
            throw error_already_set();
        }
...
class float_ : public object {
  public:
    ...
    operator float() const {
        return (float)PyFloat_AsDouble(m_ptr);
    }
    // NOLINTNEXTLINE(google-explicit-constructor)
    operator double() const {
        return (double)PyFloat_AsDouble(m_ptr);
    }
};
...
class slice : public object {
  public:
    ...
    slice(std::optional<ssize_t> start, std::optional<ssize_t> stop,
        ...
        : slice(index_to_object(start), index_to_object(stop),
                index_to_object(step)) {}
#else
...
class capsule : public object {
  public:
    ...
    capsule(const void* value, PyCapsule_Destructor destructor)
        : object(PyCapsule_New(const_cast<void*>(value), nullptr, destructor),
                 stolen_t{}) {
        if (!m_ptr) {
            throw error_already_set();
        }
    ...
    explicit capsule(void (*destructor)()) {
        m_ptr = PyCapsule_New(
            reinterpret_cast<void*>(destructor), nullptr, [](PyObject* o) {
                ...
                auto destructor =
                    reinterpret_cast<void (*)()>(PyCapsule_GetPointer(o, name));
    ...
    /// Replaces a capsule's pointer *without* calling the destructor on the
    /// existing one.
    ...
    /// Replaces a capsule's name *without* calling the destructor on the
    /// existing one.
  ...
  private:
    ...
    void initialize_with_void_ptr_destructor(const void* value, const char* name,
        ...
        if (!m_ptr ||
            PyCapsule_SetContext(m_ptr, reinterpret_cast<void*>(destructor)) != 0) {
            throw error_already_set();
        }
    }
};
...
class tuple : public object {
  public:
    ...
    explicit tuple(SzType size = 0)
        : object(PyTuple_New(ssize_t_cast(size)), stolen_t{}) {
        if (!m_ptr) {
...
// fails to compile enable_if_t<all_of<is_keyword_or_ds<Args>...>::value> part
// below (tested with ICC 2021.1 Beta 20200827).
...
class dict : public object {
  public:
    ...
    template <typename T> bool contains(T&& key) const {
        auto result = PyDict_Contains(
            m_ptr, detail::object_or_cast(std::forward<T>(key)).ptr());
        if (result == -1) {
            throw error_already_set();
        }
        return result == 1;
...
class list : public object {
  public:
    ...
    template <typename SzType = ssize_t,
              detail::enable_if_t<std::is_integral<SzType>::value, int> = 0>
    // Some compilers generate link errors when using `const SzType &` here:
    explicit list(SzType size = 0)
        : object(PyList_New(ssize_t_cast(size)), stolen_t{}) {
        if (!m_ptr) {
    ...
    detail::list_iterator end() const {
        return {*this, PyList_GET_SIZE(m_ptr)};
    }
    template <typename T> void append(T&& val) /* py-non-const */ {
        if (PyList_Append(
                m_ptr, detail::object_or_cast(std::forward<T>(val)).ptr()) != 0) {
            throw error_already_set();
        }
    }
    ...
    void insert(const IdxType& index, ValType&& val) /* py-non-const */ {
        if (PyList_Insert(
                m_ptr, ssize_t_cast(index),
                detail::object_or_cast(std::forward<ValType>(val)).ptr()) != 0) {
            throw error_already_set();
        }
    }
    void clear() /* py-non-const */ {
...
class anyset : public object {
  public:
    ...
    template <typename T> bool contains(T&& val) const {
        auto result = PySet_Contains(
            m_ptr, detail::object_or_cast(std::forward<T>(val)).ptr());
        if (result == -1) {
            throw error_already_set();
        }
        return result == 1;
...
class memoryview : public object {
  public:
    ...
    /** \rst
     ...
     \endrst */
    static memoryview
    from_buffer(void* ptr, ssize_t itemsize, const char* format,
                detail::any_container<ssize_t> shape,
                detail::any_container<ssize_t> strides, bool readonly = false);

    static memoryview
    from_buffer(const void* ptr, ssize_t itemsize, const char* format,
                detail::any_container<ssize_t> shape,
                detail::any_container<ssize_t> strides) {
        return memoryview::from_buffer(const_cast<void*>(ptr), itemsize, format,
                                       std::move(shape), std::move(strides), true);
    }

    template <typename T>
    static memoryview
    from_buffer(T* ptr, detail::any_container<ssize_t> shape,
                detail::any_container<ssize_t> strides, bool readonly = false) {
        return memoryview::from_buffer(
            reinterpret_cast<void*>(ptr), sizeof(T), format_descriptor<T>::value,
            std::move(shape), std::move(strides), readonly);
    }

    template <typename T>
    static memoryview
    from_buffer(const T* ptr, detail::any_container<ssize_t> shape,
                detail::any_container<ssize_t> strides) {
        return memoryview::from_buffer(const_cast<T*>(ptr), std::move(shape),
    ...
    static memoryview from_memory(std::string_view mem) {
        return from_memory(const_cast<char*>(mem.data()),
                           static_cast<ssize_t>(mem.size()), true);
    }
#endif
};
...
/// @cond DUPLICATE
inline memoryview
memoryview::from_buffer(void* ptr, ssize_t itemsize, const char* format,
                        detail::any_container<ssize_t> shape,
...
#define PYBIND11_MATH_OPERATOR_BINARY_INPLACE(iop, fn)                         \
    template <typename D> object object_api<D>::iop(object_api const& other) { \
        object result = reinterpret_steal<object>(                             \
            fn(derived().ptr(), other.derived().ptr()));                       \
        if (!result.ptr())                                                     \
            throw error_already_set();                                         \
