/*
    pybind11/eigen/matrix.h: Transparent conversion for dense and sparse Eigen
   matrices
...
using EigenMapSparseMatrix =
    Eigen::Map<Eigen::SparseMatrix<Scalar, Flags, StorageIndex>>;
#else
using EigenIndex = EIGEN_DEFAULT_DENSE_INDEX_TYPE;
template <typename Scalar, int Flags, typename StorageIndex>
using EigenMapSparseMatrix =
    Eigen::MappedSparseMatrix<Scalar, Flags, StorageIndex>;
#endif
...
template <bool EigenRowMajor> struct EigenConformable {
    ...
    EigenConformable(EigenIndex r, EigenIndex c, EigenIndex rstride,
        ...
        : conformable{true}, rows{r}, cols{c},
          // TODO: when Eigen bug #747 is fixed, remove the tests for
          // non-negativity. http://eigen.tuxfamily.org/bz/show_bug.cgi?id=747
    ...
    template <typename props> bool stride_compatible() const {
        ...
        // case the stride value is irrelevant). Alternatively, if any dimension
        // size is 0, the strides are not relevant (and numpy ≥ 1.23 sets the
        // strides to 0 in that case, so we need to check explicitly).
        if (negativestrides) {
            return false;
        }
        if (rows == 0 || cols == 0) {
        ...
        return (props::inner_stride == Eigen::Dynamic ||
...
template <typename Type> struct eigen_extract_stride {
    using type = Type;
};
...
template <typename Type_> struct EigenProps {
    ...
    static constexpr bool row_major = Type::IsRowMajor,
                          vector = Type::IsVectorAtCompileTime, // At least one
                                                                // dimension has
                                                                // fixed size 1
    ...
    // Eigen type.  If the array is a vector, we attempt to fit it into either
    // an Eigen 1xN or Nx1 vector (preferring the latter if it will fit in
    // either, i.e. for a fully dynamic matrix type).
    static EigenConformable<row_major> conformable(const array& a) {
        ...
        if (fixed_cols) {
            // Since this isn't a vector, cols must be != 1.  We allow this only
            // if it exactly equals the number of elements (rows is Dynamic, and
            // so 1 row is allowed).
            if (cols != n) {
                return false;
            }
            return {1, n, stride};
        } // Otherwise it's either fully dynamic, or column dynamic; both become
          // a column vector
    ...
    static constexpr auto descriptor =
        ...
        const_name("[") +
        const_name<fixed_rows>(const_name<(size_t)rows>(), const_name("m")) +
        const_name(", ") +
        ...
        // f_contiguous or c_contiguous.  We include them in the descriptor
        // output to provide some hint as to why a TypeError is occurring
        // (otherwise it can be confusing to see that a function accepts a
...
handle eigen_ref_array(Type& src, handle parent = none()) {
    ...
    // currently always copies when there is no base.  Setting the base to None
    // should be harmless.
...
struct type_caster<Type, enable_if_t<is_eigen_dense_plain<Type>::value>> {
    ...
    bool load(handle src, bool convert) {
        // If we're in no-convert mode, only load if given an array of the
        // correct type
  ...
  private:
    ...
    static handle
    cast_impl(CType* src, return_value_policy policy, handle parent) {
        switch (policy) {
...
template <typename MapType> struct eigen_map_caster {
  ...
  public:
    ...
    // matrix type.) Note that this means you need to ensure you don't destroy
    // the object in some other way (e.g. with an appropriate keep_alive, or
    // with a reference to a statically allocated matrix).
    static handle
    ...
    // these can be return types but not bound arguments).  We still provide
    // them (with an explicitly delete) so that you end up here if you try
    // anyway.
...
struct type_caster<
  ...
  private:
    ...
    // that needs to be converted).  Using a numpy temporary (rather than an
    // Eigen temporary) saves an extra copy when we need both type conversion
    // and storage order conversion.  (Note that we refuse to use this temporary
    // copy when loading an argument for a Ref<M> with M non-const, i.e. a
  ...
  public:
    bool load(handle src, bool convert) {
        ...
        // type.  If not, we can't avoid a copy (because the copy is also going
        // to do type conversion).
        bool need_copy = !isinstance<Array>(src);

        EigenConformable<props::row_major> fits;
        if (!need_copy) {
            // We don't need a converting copy, but we also need to check
            // whether the strides are compatible with the Ref's stride
            // requirements
        ...
        if (need_copy) {
            // We need to copy: If we need a mutable reference, or we're not
            // supposed to convert (either because we're in the no-convert
            // overload pass, or because we're explicitly instructed not to copy
            // (via `py::arg().noconvert()`) we have to fail loading.
  ...
  private:
    ...
    // Otherwise, if there is a two-index constructor, assume it is
    // (outer,inner) like Eigen::Stride, and use it:
    template <typename S>
    using stride_ctor_dual =
        bool_constant<!stride_ctor_default<S>::value &&
                      std::is_constructible<S, EigenIndex, EigenIndex>::value>;
    // Otherwise, if there is a one-index constructor, and just one of the
    // strides is dynamic, use it (passing whichever stride is dynamic).
    template <typename S>
...
struct type_caster<Type, enable_if_t<is_eigen_sparse<Type>::value>> {
    ...
    static_assert(!std::is_pointer<Scalar>::value,
                  PYBIND11_EIGEN_MESSAGE_POINTER_TYPES_ARE_NOT_SUPPORTED);
    using StorageIndex =
        remove_reference_t<decltype(*std::declval<Type>().outerIndexPtr())>;
    ...
    bool load(handle src, bool) {
        ...
        value = EigenMapSparseMatrix<
            Scalar, Type::Flags&(Eigen::RowMajor | Eigen::ColMajor), StorageIndex>(
            shape[0].cast<Index>(), shape[1].cast<Index>(), std::move(nnz),
    ...
    static handle cast(const Type& src, return_value_policy /* policy */,
        ...
        return matrix_type(pybind11::make_tuple(std::move(data),
                                                std::move(innerIndices),
                                                std::move(outerIndices)),
                           pybind11::make_tuple(src.rows(), src.cols()))
            .release();
    }

    PYBIND11_TYPE_CASTER(
        Type, const_name<(Type::IsRowMajor) != 0>("scipy.sparse.csr_matrix[",
