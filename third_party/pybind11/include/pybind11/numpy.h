...
#include <vector>
    ...
    !defined(PYBIND11_INTERNAL_NUMPY_1_ONLY_DETECTED)
...
/* This will be true on all flat address space platforms and allows us to reduce
   ...
   inflicting this upon the library user. Note that NumPy 2 now uses ssize_t for
   `npy_intp` to simplify this. */
static_assert(sizeof(::pybind11::ssize_t) == sizeof(Py_intptr_t),
              "ssize_t != Py_intptr_t");
static_assert(std::is_signed<Py_intptr_t>::value, "Py_intptr_t must be signed");
// We now can reinterpret_cast between py::ssize_t and Py_intptr_t (MSVC + PyPy
// cares)
...
template <> struct handle_type_name<dtype> {
    static constexpr auto name = const_name("numpy.dtype");
};

template <> struct handle_type_name<array> {
...
struct numpy_internals {
    ...
    get_type_info(const std::type_info& tinfo, bool throw_if_missing = true) {
        ...
        if (throw_if_missing) {
            pybind11_fail(std::string("NumPy type info missing for ") +
                          tinfo.name());
        }
        return nullptr;
    }
...
struct npy_api {
    enum constants {
        ...
        // NPY_BITSOF_LONG, NPY_BITSOF_LONGLONG, NPY_BITSOF_INT,
        // NPY_BITSOF_SHORT, NPY_BITSOF_CHAR and assigns the alias to the first
        // matching size, so we should check in this order.
        NPY_INT32_ = platform_lookup<std::int32_t, long, int, short>(
            NPY_LONG_, NPY_INT_, NPY_SHORT_),
        NPY_UINT32_ =
            platform_lookup<std::uint32_t, unsigned long, unsigned int, unsigned short>(
    ...
    PyObject* (*PyArray_NewFromDescr_)(PyTypeObject*, PyObject*, int,
                                       Py_intptr_t const*, Py_intptr_t const*,
                                       void*, int, PyObject*);
...
#ifdef PYBIND11_NUMPY_1_ONLY
    int (*PyArray_GetArrayParamsFromObject_)(PyObject*, PyObject*, unsigned char,
                                             PyObject**, int*, Py_intptr_t*,
                                             PyObject**, PyObject*);
#endif
  ...
  private:
    enum functions {
        ...
        // CopyInto was slot 82 and 50 was effectively an alias. NumPy 2
        // removed 82.
...
template <typename T, size_t N> struct array_info<std::array<T, N>> {
    ...
    static constexpr auto extents = const_name<array_info<T>::is_array>(
        ::pybind11::detail::concat(const_name<N>(), array_info<T>::extents),
        const_name<N>());
};
...
#if defined(__GLIBCXX__) && (__GLIBCXX__ < 20150422 || __GLIBCXX__ == 20150426 || \
                             __GLIBCXX__ == 20150623 ||                           \
                             __GLIBCXX__ == 20150626 || __GLIBCXX__ == 20160803)
           // libstdc++ < 5 (including versions 4.8.5, 4.9.3 and 4.9.4 which
           // were released after 5) don't implement is_trivially_copyable, so
           // approximate it
...
template <typename T, ssize_t Dims> class unchecked_reference {
  ...
  public:
    ...
    /// Returns the total number of bytes used by the referenced data.  Note
    /// that the actual span in memory may be larger if the referenced array has
...
class dtype : public object {
  public:
    ...
    explicit dtype(const buffer_info& info) {
        ...
        // If info.itemsize == 0, use the value calculated from the format
        // string
        m_ptr = descr
                    .strip_padding(info.itemsize != 0 ? info.itemsize
                                                      : descr.itemsize())
                    .release()
                    .ptr();
    }
    ...
    explicit dtype(int typenum)
        : object(detail::npy_api::get().PyArray_DescrFromType_(typenum),
                 stolen_t{}) {
        if (m_ptr == nullptr) {
            throw error_already_set();
        }
    ...
    static dtype from_args(const object& args) {
        ...
        if ((detail::npy_api::get().PyArray_DescrConverter_(args.ptr(), &ptr) == 0) ||
            !ptr) {
            throw error_already_set();
        }
    ...
    int num() const {
        // Note: The signature, `dtype::num` follows the naming of NumPy's
        // public Python API (i.e., ``dtype.num``), rather than its internal C
        // API (``PyArray_Descr::type_num``).
...
/// Flags for the array descriptor
  ...
  private:
    ...
    dtype strip_padding(ssize_t itemsize) {
        // Recursively strip all void fields with empty names that are generated
        // for padding fields (as of NumPy v1.11).
        if (!has_fields()) {
            return *this;
        }
        ...
        for (auto field : field_dict.attr("items")()) {
            ...
            field_descriptors.emplace_back(std::move(name),
        ...
        return dtype(std::move(names), std::move(formats), std::move(offsets),
                     itemsize);
    }
};
  ...
  public:
    ...
    array(const pybind11::dtype& dt, ShapeContainer shape, StridesContainer strides,
        ...
        auto tmp = reinterpret_steal<object>(api.PyArray_NewFromDescr_(
            ...
            // Use reinterpret_cast for PyPy on Windows (remove if fixed,
            // checked on 7.3.1)
    ...
    array(ShapeContainer shape, StridesContainer strides, const T* ptr,
        ...
        : array(pybind11::dtype::of<T>(), std::move(shape), std::move(strides),
                reinterpret_cast<const void*>(ptr), base) {}
    ...
    /// If set, the array owns the data (will be freed when the array is
    /// deleted)
    bool owndata() const {
    ...
    /// Mutable pointer to the contained data. If index is not provided, points
    /// to the beginning of the buffer. May throw if the index would lead to out
    /// of bounds access. May throw if the array is not writeable.
    ...
    /// Item count from beginning of the array to a given index (full or
    /// partial). May throw if the index would lead to out of bounds access.
     ...
     * bounds or dimensionality checking.  Will throw if the array is missing
     * the `writeable` flag.  Use with care: the array must not be destroyed or
     * reshaped for the duration of the returned object, and the caller must
     * take care not to access invalid dimensions or dimension indices.
     */
     ...
     * Use with care: the array must not be destroyed or reshaped for the
     * duration of the returned object, and the caller must take care not to
    ...
    array reshape(ShapeContainer new_shape) {
        detail::npy_api::PyArray_Dims d = {
            reinterpret_cast<Py_intptr_t*>(new_shape->data()),
            int(new_shape->size())};
        auto new_array = reinterpret_steal<array>(
    ...
    /// In case of an error, nullptr is returned and the Python error is
    /// cleared.
...
class array_t : public array {
  ...
  public:
    ...
    explicit array_t(const buffer_info& info, handle base = handle())
        : array(info, base) {}
    ...
    /// not, try to convert it).  In case of an error, nullptr is returned and
    /// the Python error is cleared.
...
struct format_descriptor<T, detail::enable_if_t<detail::array_info<T>::is_array>> {
    static std::string format() {
        ...
        static constexpr auto extents =
            const_name("(") + array_info<T>::extents + const_name(")");
...
struct npy_format_descriptor_name<T, enable_if_t<std::is_floating_point<T>::value>> {
    static constexpr auto
        name = const_name < std::is_same<T, float>::value ||
...
struct npy_format_descriptor_name<T, enable_if_t<is_complex<T>::value>> {
    static constexpr auto
        name = const_name < std::is_same<typename T::value_type, float>::value ||
...
struct npy_format_descriptor<
  ...
  public:
    ...
    static pybind11::dtype dtype() {
        return pybind11::dtype(/*typenum*/ value);
    }
};
...
struct npy_format_descriptor<T, enable_if_t<array_info<T>::is_array>> {
  ...
  public:
    ...
    static constexpr auto name = const_name("(") + array_info<T>::extents +
                                 const_name(")") + base_descr::name;
    static pybind11::dtype dtype() {
        list shape;
        array_info<T>::append_extents(shape);
...
register_structured_dtype(any_container<field_descriptor> fields,
    ...
    auto* dtype_ptr = pybind11::dtype(std::move(names), std::move(formats),
                                      std::move(offsets), itemsize)
                          .release()
                          .ptr();
    ...
    // isn't guaranteed to work due to
    // https://github.com/numpy/numpy/issues/9049
    oss << "^T{";
    for (auto& field : ordered_fields) {
...
#    if defined(_MSC_VER) && !defined(__clang__) // MSVC is not as eager to
                                                 // expand macros, hence this
                                                 // workaround
...
#    define PYBIND11_MAP2_LIST1(f, t, x1, x2, peek, ...)                       \
        f(t, x1, x2) PYBIND11_MAP2_LIST_NEXT(peek, PYBIND11_MAP2_LIST0)(       \
            f, t, peek, __VA_ARGS__)
...
// C-contiguous (`c_trivial`) or Fortran-contiguous
...
broadcast_trivial broadcast(const std::array<buffer_info, N>& buffers,
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        for (auto shape_iter = buffers[i].shape.rbegin(); shape_iter != end;
            ...
            // Each input dimension can either be 1 or `n`, but `n` values must
            // match across buffers
            if (dim_size_out == 1) {
    ...
    for (size_t i = 0; i < N && (trivial_broadcast_c || trivial_broadcast_f); ++i) {
        ...
        // Check for Fortran contiguity (if previous inputs were also F
        // contiguous)
...
template <typename T> struct vectorize_arg {
    ...
    static constexpr bool vectorize =
        ...
        satisfies_none_of<call_type, std::is_pointer, std::is_array,
                          is_std_array, std::is_enum>::value &&
        (!std::is_reference<T>::value ||
         (std::is_lvalue_reference<T>::value && std::is_const<call_type>::value));
    // Accept this type: an array for vectorized types, otherwise the type
    // as-is:
    using type =
        conditional_t<vectorize, array_t<remove_cv_t<call_type>, array::forcecast>, T>;
};
...
struct vectorize_returned_array {
    ...
    static Type
    create(broadcast_trivial trivial, const std::vector<ssize_t>& shape) {
...
// NVCC for some reason breaks if NVectorized is private
  ...
  private:
    ...
    // 2017 15.9.4), when compiling with "/permissive-" flag when arg_call_types
    // is manually inlined.
    ...
    // Runs a vectorized function given arguments tuple and three index
    // sequences:
    //     - Index is the full set of 0 ... (N-1) argument indices;
    //     - VIndex is the subset of argument indices with vectorized
    //     parameters, letting us access
    //       vectorized arguments (anything not in this sequence is passed
    //       through)
    //     - BIndex is a incremental sequence (beginning at 0) of the same size
    //     as VIndex, so that
    //       we can store vectorized buffer_infos in an array (argument VIndex
    //       has its buffer at index BIndex in the array).
    template <size_t... Index, size_t... VIndex, size_t... BIndex>
    object run(typename vectorize_arg<Args>::type&... args,
               index_sequence<Index...> i_seq, index_sequence<VIndex...> vi_seq,
               index_sequence<BIndex...> bi_seq) {
        // Pointers to values the function was called with; the vectorized ones
        // set here will start out as array_t<T> pointers, but they will be
        // changed them to T pointers before we make call the wrapped function.
        // Non-vectorized pointers are left as-is.
        ...
        if (size == 1 && ndim == 0) {
            ...
            return cast(returned_array::call(
        ...
        if (trivial == broadcast_trivial::non_trivial) {
            apply_broadcast(buffers, params, mutable_data, size, shape, i_seq,
                            vi_seq, bi_seq);
        } else {
            apply_trivial(buffers, params, mutable_data, size, i_seq, vi_seq,
                          bi_seq);
        }
    ...
    void apply_trivial(std::array<buffer_info, NVectorized>& buffers,
        ...
        // Initialize an array of mutable byte references and sizes with
        // references set to the appropriate pointer in `params`; as we iterate,
        // we'll increment each pointer by its size (except for singletons,
        // which get an increment of 0).
    ...
    void apply_broadcast(std::array<buffer_info, NVectorized>& buffers,
        ...
        for (size_t i = 0; i < size; ++i, ++input_iter) {
            ...
            returned_array::call(
                out, i, f,
                *reinterpret_cast<param_n_t<Index>*>(std::get<Index>(params))...);
        }
    }
};
