...
template <> class type_caster<PyObject> {
  public:
    static constexpr auto name = const_name("object"); // See discussion under
                                                       // PR #4601.
    ...
    static handle
    cast(PyObject* src, return_value_policy policy, handle /*parent*/) {
        if (src == nullptr) {
    ...
    template <typename T> using cast_op_type = PyObject*;
