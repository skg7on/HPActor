...
/* https://stackoverflow.com/questions/46798456/handling-gccs-noexcept-type-warning
   ...
   It is only actually needed in a couple places, but apparently GCC 7
   "generates this warning if and only if the first template instantiation ...
   involves noexcept" [stackoverflow], therefore it could get triggered from
   seemingly random places, depending on user code. No other GCC version
...
inline std::string replace_newlines_and_squash(const char* text) {
    ...
    // Replace characters in whitespaces array with spaces and squash
    // consecutive spaces
    while (*text != '\0') {
...
class cpp_function : public function {
  public:
    ...
    /// Construct a cpp_function from a class method (non-const, no
    /// ref-qualifier)
    ...
    cpp_function(Return (Class::*f)(Arg...), const Extra&... extra) {
        initialize(
            [f](Class* c, Arg... args) -> Return {
                return (c->*f)(std::forward<Arg>(args)...);
            },
            (Return (*)(Class*, Arg...)) nullptr, extra...);
    }

    /// Construct a cpp_function from a class method (non-const, lvalue
    /// ref-qualifier) A copy of the overload for non-const functions without
    /// explicit ref-qualifier but with an added `&`.
    template <typename Return, typename Class, typename... Arg, typename... Extra>
    // NOLINTNEXTLINE(google-explicit-constructor)
    cpp_function(Return (Class::*f)(Arg...) &, const Extra&... extra) {
        initialize(
    ...
    /// Construct a cpp_function from a class method (const, lvalue
    /// ref-qualifier) A copy of the overload for const functions without
    /// explicit ref-qualifier but with an added `&`.
    template <typename Return, typename Class, typename... Arg, typename... Extra>
    // NOLINTNEXTLINE(google-explicit-constructor)
    cpp_function(Return (Class::*f)(Arg...) const&, const Extra&... extra) {
  ...
  protected:
    struct InitializingFunctionRecordDeleter {
        ...
        // strings and takes care of cleaning up in case of exceptions. So pass
        // `false` to `free_strings`.
    ...
    void initialize(Func&& f, Return (*)(Args...), const Extra&... extra) {
        ...
        /* Store the function including any extra state it might have (e.g. a
        ...
        static_assert(
            expected_num_args<Extra...>(sizeof...(Args), cast_in::args_pos >= 0,
                                        cast_in::has_kwargs),
            "The number of argument annotations does not match the number of function arguments");

        /* Dispatch code which converts function arguments and performs the
        ...
        rec->nargs_pos = cast_in::args_pos >= 0
                             ...
                             : sizeof...(Args) - cast_in::has_kwargs; // Will
                                                                      // get
                                                                      // reduced
                                                                      // more if
                                                                      // we have
        ...
        {
            constexpr bool has_kw_only_args =
                               any_of<std::is_same<kw_only, Extra>...>::value,
                           has_pos_only_args =
                               any_of<std::is_same<pos_only, Extra>...>::value,
            ...
            static_assert(!(has_kw_only_args && has_pos_only_args) ||
                              pos_only_pos < kw_only_pos,
         ...
         */
        // Pass on the ownership over the `unique_rec` to `initialize_generic`.
        // `rec` stays valid.
        initialize_generic(std::move(unique_rec), signature.text, types.data(),
                           sizeof...(Args));

        /* Stash some additional information used by an important optimization
         * in 'functional.h' */
    ...
    // Utility class that keeps track of all duplicated strings, and cleans them
    // up in its destructor, unless they are released. Basically a RAII-solution
    ...
    void initialize_generic(unique_function_record&& unique_rec, const char* text,
        ...
        // Do NOT receive `unique_rec` by value. If this function fails to move
        // out the unique_ptr, we do not want this to destruct the pointer.
        // `initialize` (the caller) still relies on the pointee being alive
        // after this call. Only move out if a `capsule` is going to keep it
        // alive.
        auto* rec = unique_rec.get();

        // Keep track of strdup'ed strings, and clean them up as long as the
        // function's capsule has not taken ownership yet (when
        // `unique_rec.release()` is called). Note: This cannot easily be fixed
        // by a `unique_ptr` with custom deleter, because the strings are only
        // referenced before strdup'ing. So only *after* the following block
        // could `destruct` safely be called, but even then, `repr` could still
        // throw in the middle of copying all strings.
        ...
        for (auto& a : rec->args) {
            ...
            if (a.descr) {
                ...
                a.descr =
                    guarded_strdup(repr(a.value).cast<std::string>().c_str());
            }
        }
    ...
    !defined(PYBIND11_DISABLE_NEW_STYLE_INIT_WARNING)
        if (rec->is_constructor && !rec->is_new_style_constructor) {
            const auto class_name = detail::get_fully_qualified_tp_name(
                (PyTypeObject*)rec->scope.ptr());
        ...
        for (const auto* pc = text; *pc != '\0'; ++pc) {
            ...
            if (c == '{') {
                ...
                if (arg_index < rec->args.size() && rec->args[arg_index].name) {
                    ...
                    signature += "arg" + std::to_string(arg_index -
                                                        (rec->is_method ? 1 : 0));
                }
                signature += ": ";
            } else if (c == '}') {
                // Write default value if available.
                if (!is_starred && arg_index < rec->args.size() &&
                    rec->args[arg_index].descr) {
                    signature += " = ";
                    signature += detail::replace_newlines_and_squash(
                        rec->args[arg_index].descr);
                }
                // Separator for positional-only arguments (placed after the
                // argument, rather than before like *
                if (rec->nargs_pos_only > 0 &&
                    (arg_index + 1) == rec->nargs_pos_only) {
                    signature += ", /";
                }
                if (!is_starred) {
                    arg_index++;
                ...
                if (auto* tinfo = detail::get_type_info(*t)) {
                    ...
                    // A new-style `__init__` takes `self` as
                    // `value_and_holder`. Rewrite it to the proper class type.
                    signature +=
                        rec->scope.attr("__module__").cast<std::string>() + "." +
                        rec->scope.attr("__qualname__").cast<std::string>();
                } else {
                    signature += detail::quote_cpp_type_name(
                        detail::clean_type_id(t->name()));
                }
            } else {
                signature += c;
            }
        ...
        if (arg_index != args - rec->has_args - rec->has_kwargs ||
            types[type_index] != nullptr) {
        ...
        if (rec->sibling && PYBIND11_INSTANCE_METHOD_CHECK(rec->sibling.ptr())) {
            rec->sibling =
                PYBIND11_INSTANCE_METHOD_GET_FUNCTION(rec->sibling.ptr());
        }
            ...
            if (PyCFunction_Check(rec->sibling.ptr())) {
                ...
                if (!isinstance<capsule>(self)) {
                    ...
                    if (detail::is_function_record_capsule(rec_capsule)) {
                        ...
                        /* Never append a method to an overload chain of a
                           parent class; instead, hide the parent's overloads in
                           this case */
        ...
        if (!chain) {
            ...
            capsule rec_capsule(
                unique_rec.release(), detail::get_function_record_capsule_name(),
            ...
            m_ptr = PyCFunction_NewEx(rec->def, rec_capsule.ptr(),
                                      scope_module.ptr());
            if (!m_ptr) {
                pybind11_fail(
            ...
            if (rec->prepend) {
                // Beginning of chain; we need to replace the capsule's current
                // head-of-the-chain pointer with this one, then make this one
                // point to the previous head of the chain.
                chain_start = rec;
                rec->next = chain;
                auto rec_capsule = reinterpret_borrow<capsule>(
                    ((PyCFunctionObject*)m_ptr)->m_self);
        ...
        for (auto* it = chain_start; it != nullptr; it = it->next) {
            ...
            if (it->doc && it->doc[0] != '\0' &&
                options::show_user_defined_docstrings()) {
                // If we're appending another docstring, and aren't printing
                // function signatures, we need to append a newline first:
                if (!options::show_function_signatures()) {
                    if (first_user_def) {
        ...
        // Install docstring if it's non-empty (when at least one option is
        // enabled)
        func->m_ml->ml_doc = signatures.empty()
                                 ? nullptr
                                 : PYBIND11_COMPAT_STRDUP(signatures.c_str());
...
// If this is running on 3.9.0, we have to work around a bug.
        ...
        while (rec) {
            ...
            // During initialization, these strings might not have been copied
            // yet, so they cannot be freed. Once the function has been created,
            // they can. Check `make_function_record` for more details.
            if (free_strings) {
...
// See https://github.com/python/cpython/pull/22670
    ...
    static PyObject*
    dispatcher(PyObject* self, PyObject* args_in, PyObject* kwargs_in) {
        using namespace detail;
        ...
        const function_record *overloads = reinterpret_cast<function_record*>(
                                  PyCapsule_GetPointer(
                                      self, get_function_record_capsule_name())),
                              *current_overload = overloads;
        assert(overloads != nullptr);

        /* Need to know how many arguments + keyword arguments there are to pick
           the right overload */
        ...
        if (overloads->is_constructor) {
            ...
            auto* const tinfo =
                get_type_info((PyTypeObject*)overloads->scope.ptr());
            auto* const pi = reinterpret_cast<instance*>(parent.ptr());
            self_value_and_holder = pi->get_value_and_holder(tinfo, true);

            // If this value is already registered it must mean __init__ is
            // invoked multiple times; we really can't support that in C++, so
            // just ignore the second __init__.
        ...
        try {
            // We do this in two passes: in the first pass, we load arguments
            // with `convert=false`; in the second, we allow conversion (except
            // for arguments with an explicit py::arg().noconvert()).  This lets
            // us prefer calls without conversion, with conversion as a
            // fallback.
            std::vector<function_call> second_pass;

            // However, if there are no overloads, we can just skip the
            // no-convert pass entirely
            const bool overloaded = current_overload != nullptr &&
                                    current_overload->next != nullptr;

            for (; current_overload != nullptr;
                 current_overload = current_overload->next) {
                /* For each overload:
                   1. Copy all positional arguments we were given, also checking
                   to make sure that named positional arguments weren't *also*
                   specified via kwarg.
                   2. If we weren't given enough, try to make up the omitted
                   ones by checking whether they were provided by a kwarg
                   matching the `py::arg("name")` name. If so, use it (and
                   remove it from kwargs); if not, see if the function binding
                      provided a default that we can use.
                   3. Ensure that either all keyword arguments were "consumed",
                   or that the function takes a kwargs argument to accept
                   unconsumed kwargs.
                   4. Any positional arguments still left get put into a tuple
                   (for args), and any leftover kwargs get put into a dict.
                   5. Pack everything into a vector; if we have py::args or
                   py::kwargs, they are an extra tuple or dict at the end of the
                   positional arguments.
                   6. Call the function call dispatcher (function_record::impl)

                   If one of these fail, move on to the next overload and keep
                   trying until we get a result other than
                   PYBIND11_TRY_NEXT_OVERLOAD.
                 */

                const function_record& func = *current_overload;
                size_t num_args = func.nargs; // Number of positional arguments
                                              // that we need
                if (func.has_args) {
                ...
                if (n_args_in < pos_args && func.args.size() < pos_args) {
                    continue; // Not enough positional arguments given, and not
                              // enough defaults to fill in the blanks
                }
                ...
                if (func.is_new_style_constructor) {
                    // The `value` may have been preallocated by an old-style
                    // `__init__` if it was a preceding candidate for overload
                    // resolution.
                ...
                for (; args_copied < args_to_copy; ++args_copied) {
                    const argument_record* arg_rec =
                        args_copied < func.args.size() ? &func.args[args_copied]
                                                       : nullptr;
                ...
                if (bad_arg) {
                    continue; // Maybe it was meant for another overload (issue
                              // #688)
                }

                // Keep track of how many position args we copied out in case we
                // need to come back to copy the rest into a py::args argument.
                size_t positional_args_copied = args_copied;

                // We'll need to copy this if we steal some kwargs for defaults
                dict kwargs = reinterpret_borrow<dict>(kwargs_in);

                // 1.5. Fill in any missing pos_only args from defaults if they
                // exist
                if (args_copied < func.nargs_pos_only) {
                    ...
                    if (args_copied < func.nargs_pos_only) {
                        continue; // Not enough defaults to fill the positional
                                  // arguments
                    }
                }

                // 2. Check kwargs and, failing that, defaults that may help
                // complete the list
                if (args_copied < num_args) {
                    ...
                    for (; args_copied < num_args; ++args_copied) {
                        ...
                        if (value) {
                            ...
                            if (!copied_kwargs) {
                                kwargs = reinterpret_steal<dict>(
                                    PyDict_Copy(kwargs.ptr()));
                                copied_kwargs = true;
                            }
                            if (PyDict_DelItemString(kwargs.ptr(), arg_rec.name) ==
                                -1) {
                        ...
                        if (value) {
                            // If we're at the py::args index then first insert
                            // a stub for it to be replaced later
                    ...
                    if (args_copied < num_args) {
                        continue; // Not enough arguments, defaults, or kwargs
                                  // to fill the positional arguments
                    }
                }

                // 3. Check everything was consumed (unless we have a kwargs
                // arg)
                if (kwargs && !kwargs.empty() && !func.has_kwargs) {
                    continue; // Unconsumed kwargs, but no py::kwargs argument
                              // to accept them
                }

                // 4a. If we have a py::args argument, create a new tuple with
                // leftovers
                if (func.has_args) {
                    tuple extra_args;
                    if (args_to_copy == 0) {
                        // We didn't copy out any position arguments from the
                        // args_in tuple, so we can reuse it directly without
                        // copying:
                        ...
                        for (size_t i = 0; i < args_size; ++i) {
                            extra_args[i] = PyTuple_GET_ITEM(
                                args_in, positional_args_copied + i);
                        }
                    }
                ...
                if (func.has_kwargs) {
                    if (!kwargs.ptr()) {
                        kwargs = dict(); // If we didn't get one, send an empty
                                         // one
                    }
...
// it in `call.args` all along.
                ...
                if (overloaded) {
                    // We're in the first no-convert pass, so swap out the
                    // conversion flags for a set of all-false flags.  If the
                    // call fails, we'll swap the flags back in for the
                    // conversion-allowed call below.
                ...
                if (overloaded) {
                    // The (overloaded) call failed; if the call has at least
                    // one argument that permits conversion (i.e. it hasn't been
                    // explicitly specified `.noconvert()`) then add this call
                    // to the list of second pass overloads to try.
                    for (size_t i = func.is_method ? 1 : 0; i < pos_args; i++) {
                        if (second_pass_convert[i]) {
                            // Found one: swap the converting flags back in and
                            // store the call for the second pass.
                            call.args_convert.swap(second_pass_convert);
            ...
            if (overloaded && !second_pass.empty() &&
                ...
                for (auto& call : second_pass) {
                    ...
                    if (result.ptr() != PYBIND11_TRY_NEXT_OVERLOAD) {
                        // The error reporting logic below expects
                        // 'current_overload' to be valid, as it would be if
                        // we'd encountered this failure in the first-pass loop.
                        if (!result) {
...
template <> struct handle_type_name<cpp_function> {
...
class module_ : public object {
  public:
    ...
    ``Func`` can be a plain C++ function, a function pointer, or a lambda
    function. For details on the ``Extra&& ... extra`` argument, see section
    :ref:`extras`.
    \endrst */
    template <typename Func, typename... Extra>
    module_& def(const char* name_, Func&& f, const Extra&... extra) {
        ...
        // NB: allow overwriting here because cpp_function sets up a chain with
        // the intention of overwriting (and has already checked internally that
        // it isn't overwriting non-functions).
    ...
    add_object(const char* name, handle obj, bool overwrite = false) {
    ...
    using module_def = PyModuleDef; // TODO: Can this be removed (it was needed
                                    // only for Python 2)?
    ...
    \endrst */
    static module_
    create_extension_module(const char* name, const char* doc, module_def* def,
                            mod_gil_not_used gil_not_used = mod_gil_not_used(false)) {
        ...
        // TODO: Should be reinterpret_steal for Python 3, but Python also
        // steals it again when
...
template <> struct handle_type_name<module_> {
    static constexpr auto name = const_name("module");
};

PYBIND11_NAMESPACE_END(detail)

// When inside a namespace (or anywhere as long as it's not the first item on a
// line), C++20 allows "module" to be used. This is provided for backward
// compatibility, and for simplicity, if someone wants to use py::module for
// example, that is perfectly safe.
...
inline dict globals() {
    ...
    return p ? reinterpret_steal<dict>(p)
             : reinterpret_borrow<dict>(
                   module_::import("__main__").attr("__dict__").ptr());
#else
    PyObject* p = PyEval_GetGlobals();
    return reinterpret_borrow<dict>(
        p ? p : module_::import("__main__").attr("__dict__").ptr());
#endif
}
...
object make_simple_namespace(Args&&... args_) {
    return module_::import("types").attr("SimpleNamespace")(
  ...
  protected:
    void initialize(const type_record& rec) {
        ...
        if (rec.bases.size() > 1 || rec.multiple_inheritance) {
            ...
            // The parent can no longer be a simple type if it has MI and has a
            // child
            parent_tinfo->simple_type =
                parent_tinfo->simple_type && parent_simple_ancestors;
        }

        if (rec.module_local) {
            // Stash the local typeinfo and loader so that external modules can
            // access it.
    ...
    void def_property_static_impl(const char* name, handle fget, handle fset,
        ...
        const auto is_static =
            (rec_func != nullptr) && !(rec_func->is_method && rec_func->scope);
...
inline void
add_class_method(object& cls, const char* name_, const cpp_function& cf) {
    cls.attr(cf.name()) = cf;
    if (std::strcmp(name_, "__eq__") == 0 &&
        !cls.attr("__dict__").contains("__hash__")) {
...
class class_ : public detail::generic_type {
  ...
  public:
    ...
    class_(handle scope, const char* name, const Extra&... extra) {
        ...
        // parameters
        static_assert(
            none_of<is_pyobject<Extra>...>::value || // no base class arguments,
                                                     // or:
                (constexpr_sum(is_pyobject<Extra>::value...) == 1 && // Exactly
                                                                     // one base
                 constexpr_sum(is_base<options>::value...) == 0 &&   // no
                                                                     // template
                                                                     // option
                                                                     // bases
        ...
        /* Register base classes specified via template arguments to class_, if
         * any */
    ...
    class_&
    def(const detail::initimpl::constructor<Args...>& init, const Extra&... extra) {
    ...
    template <typename... Args, typename... Extra>
    class_&
    def(detail::initimpl::pickle_factory<Args...>&& pf, const Extra&... extra) {
    ...
    template <typename C, typename D, typename... Extra>
    class_&
    def_readonly(const char* name, const D C::* pm, const Extra&... extra) {
        static_assert(
            std::is_same<C, type>::value || std::is_base_of<C, type>::value,
    ...
    class_& def_readwrite_static(const char* name, D* pm, const Extra&... extra) {
        cpp_function fget([pm](const object&) -> const D& { return *pm; },
                          scope(*this)),
    ...
    template <typename D, typename... Extra>
    class_&
    def_readonly_static(const char* name, const D* pm, const Extra&... extra) {
        cpp_function fget([pm](const object&) -> const D& { return *pm; },
                          scope(*this));
    ...
    class_& def_property_readonly(const char* name, const Getter& fget,
        ...
        return def_property_readonly(
            name, cpp_function(method_adaptor<type>(fget)),
            return_value_policy::reference_internal, extra...);
    }
    ...
    template <typename... Extra>
    class_&
    def_property_readonly_static(const char* name, const cpp_function& fget,
                                 const Extra&... extra) {
    ...
    class_& def_property(const char* name, const Getter& fget,
                         const Setter& fset, const Extra&... extra) {
    ...
    class_& def_property_static(const char* name, const cpp_function& fget,
        ...
        if (rec_fget) {
            char* doc_prev = rec_fget->doc; /* 'extra' field may include a
  ...
  private:
    ...
    /// Initialize holder object, variant 2: try to construct from existing
    /// holder object, if possible
    static void
    init_holder(detail::instance* inst, detail::value_and_holder& v_h,
                const holder_type* holder_ptr,
    ...
    static void dealloc(detail::value_and_holder& v_h) {
        // We could be deallocating because we are cleaning up after a Python
        // exception. If so, the Python error indicator will be set. We need to
        // clear that before running the destructor, in case the destructor code
...
detail::initimpl::pickle_factory<GetState, SetState>
...
struct enum_base {
    ...
    PYBIND11_NOINLINE void init(bool is_arithmetic, bool is_convertible) {
        ...
        m_base.attr("name") =
            property(cpp_function(&enum_name, name("name"), is_method(m_base)));

        m_base.attr("__str__") = cpp_function(
            [](handle arg) -> str {
                object type_name = type::handle_of(arg).attr("__name__");
                return pybind11::str("{}.{}").format(std::move(type_name),
                                                     enum_name(arg));
            },
            name("__str__"), is_method(m_base));
                    ...
                    [](handle arg) -> std::string {
                        ...
                        for (auto kv : entries) {
                            ...
                            if (!comment.is_none()) {
                                docstring += " : ";
                                docstring +=
                                    pybind11::str(comment).cast<std::string>();
                            }
                        }
                        return docstring;
                    },
...
#undef PYBIND11_ENUM_OP_STRICT
    ...
    PYBIND11_NOINLINE void
    value(char const* name_, object value, const char* doc = nullptr) {
        dict entries = m_base.attr("__entries");
        str name(name_);
        if (entries.contains(name)) {
            std::string type_name = (std::string)str(m_base.attr("__name__"));
            throw value_error(std::move(type_name) + ": element \"" +
                              std::string(name_) + "\" already exists!");
        }
...
template <> struct equivalent_integer<true, 1> {
    using type = int8_t;
};
template <> struct equivalent_integer<false, 1> {
    using type = uint8_t;
};
template <> struct equivalent_integer<true, 2> {
    using type = int16_t;
};
template <> struct equivalent_integer<false, 2> {
    using type = uint16_t;
};
template <> struct equivalent_integer<true, 4> {
    using type = int32_t;
};
template <> struct equivalent_integer<false, 4> {
    using type = uint32_t;
};
template <> struct equivalent_integer<true, 8> {
    using type = int64_t;
};
template <> struct equivalent_integer<false, 8> {
    using type = uint64_t;
};
...
template <typename Type> class enum_ : public class_<Type> {
  public:
    ...
    using Scalar =
        detail::conditional_t<detail::any_of<detail::is_std_char_type<Underlying>,
    ...
    enum_(const handle& scope, const char* name, const Extra&... extra)
        ...
        attr("__setstate__") = cpp_function(
            ...
            detail::is_new_style_constructor(), pybind11::name("__setstate__"),
            is_method(*this), arg("state"));
    }
...
all_type_info_get_cache(PyTypeObject* type) {
    ...
    if (res.second) {
        // New cache entry created; set up a weak reference to automatically
        // remove it if the type gets destroyed:
        weakref((PyObject*)type, cpp_function([type](handle wr) {
                    with_internals([type](internals& internals) {
                        internals.registered_types_py.erase(type);

                        // TODO consolidate the erasure code in
                        // pybind11_meta_dealloc() in class.h
                        auto& cache = internals.inactive_override_cache;
                        for (auto it = cache.begin(), last = cache.end();
                             it != last;) {
 ...
 */
template <typename Access, return_value_policy Policy, typename Iterator,
...
iterator make_iterator_impl(Iterator first, Sentinel last, Extra&&... extra) {
    using state =
        detail::iterator_state<Access, Policy, Iterator, Sentinel, ValueType, Extra...>;
...
make_iterator(Iterator first, Sentinel last, Extra&&... extra) {
    return detail::make_iterator_impl<detail::iterator_access<Iterator>, Policy,
        ...
        std::forward<Iterator>(first), std::forward<Sentinel>(last),
        std::forward<Extra>(extra)...);
}

/// Makes a python iterator over the keys (`.first`) of a iterator over pairs
/// from a first and past-the-end InputIterator.
template <return_value_policy Policy = return_value_policy::reference_internal,
          typename Iterator, typename Sentinel,
          typename KeyType = typename detail::iterator_key_access<Iterator>::result_type,
          typename... Extra>
typing::Iterator<KeyType>
make_key_iterator(Iterator first, Sentinel last, Extra&&... extra) {
    return detail::make_iterator_impl<detail::iterator_key_access<Iterator>, Policy,
                                      Iterator, Sentinel, KeyType, Extra...>(
        std::forward<Iterator>(first), std::forward<Sentinel>(last),
        std::forward<Extra>(extra)...);
}

/// Makes a python iterator over the values (`.second`) of a iterator over pairs
/// from a first and past-the-end InputIterator.
template <return_value_policy Policy = return_value_policy::reference_internal,
          typename Iterator, typename Sentinel,
          ...
          typename... Extra>
typing::Iterator<ValueType>
make_value_iterator(Iterator first, Sentinel last, Extra&&... extra) {
    return detail::make_iterator_impl<detail::iterator_value_access<Iterator>, Policy,
                                      Iterator, Sentinel, ValueType, Extra...>(
        std::forward<Iterator>(first), std::forward<Sentinel>(last),
        std::forward<Extra>(extra)...);
}
...
typing::Iterator<ValueType> make_iterator(Type& value, Extra&&... extra) {
    return make_iterator<Policy>(std::begin(value), std::end(value),
                                 std::forward<Extra>(extra)...);
}
...
/// Makes an iterator over the values (`.second`) of a stl map-like container
/// supporting `std::begin()`/`std::end()`
template <return_value_policy Policy = return_value_policy::reference_internal, typename Type,
...
void implicitly_convertible() {
    struct set_flag {
        bool& flag;
        explicit set_flag(bool& flag_) : flag(flag_) {
            flag_ = true;
        }
        ~set_flag() {
            flag = false;
...
template <typename type> class exception : public object {
  public:
    ...
    exception(handle scope, const char* name, handle base = PyExc_Exception) {
        ...
        m_ptr = PyErr_NewException(const_cast<char*>(full_name.c_str()),
                                   base.ptr(), nullptr);
...
template <> struct handle_type_name<exception<void>> {
...
/**
 * Registers a Python exception in `m` of the given `name` and installs a
 * translator to translate the C++ exception to the created Python exception
 * using the what() method. This is intended for simple exception translations;
 * for more complex translation, register the exception object and translator
 * directly.
 */
template <typename CppException>
exception<CppException>&
register_exception(handle scope, const char* name, handle base = PyExc_Exception) {
    return detail::register_exception_impl<CppException>(scope, name, base,
                                                         false /* isLocal */);
}
 ...
 * Registers a Python exception in `m` of the given `name` and installs a
 * translator to translate the C++ exception to the created Python exception
 * using the what() method. This translator will only be used for exceptions
 * that are thrown in this module and will be tried before global exception
 * translators, including those registered with register_exception. This is
 * intended for simple exception translations; for more complex translation,
 * register the exception object and translator directly.
 */
template <typename CppException>
exception<CppException>& register_local_exception(handle scope, const char* name,
...
inline function get_type_override(const void* this_ptr,
                                  const type_info* this_type, const char* name) {
...
/** \rst
 ...
 to by the this_ptr.
  ...
  :this_ptr: The pointer to the object the overridden method should be retrieved
 for. This should be the first non-trampoline class encountered in the
 inheritance chain. :name: The name of the overridden Python method to retrieve.
  :return: The Python method by this name from the object or an empty function
 wrapper.
 \endrst */
...
#define PYBIND11_OVERRIDE_NAME(ret_type, cname, name, fn, ...)                 \
    do {                                                                       \
        PYBIND11_OVERRIDE_IMPL(PYBIND11_TYPE(ret_type), PYBIND11_TYPE(cname),  \
                               name, __VA_ARGS__);                             \
...
/** \rst
    ...
    :c:macro:`PYBIND11_OVERRIDE_NAME`, except that it throws if no override can
be found.
\endrst */
...
#define PYBIND11_OVERRIDE(ret_type, cname, fn, ...)                            \
    PYBIND11_OVERRIDE_NAME(PYBIND11_TYPE(ret_type), PYBIND11_TYPE(cname), #fn, \
                           fn, __VA_ARGS__)

/** \rst
    Macro for pure virtual functions, this function is identical to
:c:macro:`PYBIND11_OVERRIDE`, except that it throws if no override can be found.
\endrst */
#define PYBIND11_OVERRIDE_PURE(ret_type, cname, fn, ...)                       \
    PYBIND11_OVERRIDE_PURE_NAME(PYBIND11_TYPE(ret_type), PYBIND11_TYPE(cname), \
                                #fn, fn, __VA_ARGS__)
...
inline function
get_type_overload(const void* this_ptr, const detail::type_info* this_type,
                  const char* name) {
...
#define PYBIND11_OVERLOAD_NAME(ret_type, cname, name, fn, ...)                 \
    PYBIND11_OVERRIDE_NAME(PYBIND11_TYPE(ret_type), PYBIND11_TYPE(cname),      \
                           name, fn, __VA_ARGS__)
...
#define PYBIND11_OVERLOAD_PURE(ret_type, cname, fn, ...)                       \
    PYBIND11_OVERRIDE_PURE(PYBIND11_TYPE(ret_type), PYBIND11_TYPE(cname), fn,  \
                           __VA_ARGS__);
