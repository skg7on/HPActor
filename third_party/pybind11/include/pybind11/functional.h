...
struct func_handle {
    ...
    // This triggers a syntax error under very special conditions (very weird
    // indeed).
    explicit
#endif
...
struct type_caster<std::function<Return(Args...)>> {
  ...
  public:
    bool load(handle src, bool convert) {
        ...
        if (auto cfunc = func.cpp_function()) {
            ...
            if (cfunc_self == nullptr) {
                ...
                // Check that we can safely reinterpret the capsule into a
                // function_record
    ...
    static handle
    cast(Func&& f_, return_value_policy policy, handle /* parent */) {
        if (!f_) {
            return none().release();
        }
