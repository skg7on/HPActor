    ...
    static handle cast(T&& src, return_value_policy policy, handle parent) {
        ...
        for (auto&& kv : src) {
            auto key = reinterpret_steal<object>(key_conv::cast(
                detail::forward_like<T>(kv.first), policy_key, parent));
            auto value = reinterpret_steal<object>(value_conv::cast(
                detail::forward_like<T>(kv.second), policy_value, parent));
            if (!key || !value) {
                return handle();
            }
  ...
  public:
    ...
    static handle cast(T&& src, return_value_policy policy, handle parent) {
        ...
        for (auto&& value : src) {
            ...
            PyList_SET_ITEM(l.ptr(), index++, value_.release().ptr()); // steals
                                                                       // a
                                                                       // reference
        }
        return l.release();
    ...
    PYBIND11_TYPE_CASTER(Type, const_name("list[") + value_conv::name +
                                   const_name("]"));
};
  ...
  public:
    ...
    PYBIND11_TYPE_CASTER(
        ...
        const_name<Resizable>(const_name(""), const_name("Annotated[")) +
            ...
            const_name<Resizable>(const_name(""), const_name(", FixedSize(") +
                                                      const_name<Size>() +
                                                      const_name(")]")));
};
...
struct optional_caster {
    ...
    PYBIND11_TYPE_CASTER(Type, const_name("Optional[") + value_conv::name +
                                   const_name("]"));
};
...
template <template <typename...> class Variant> struct visit_helper {
    ...
    static auto
    call(Args&&... args) -> decltype(visit(std::forward<Args>(args)...)) {
...
struct variant_caster<V<Ts...>> {
    ...
    bool load(handle src, bool convert) {
        ...
        // resolution. E.g. `py::int_(1).cast<variant<double, int>>()` needs to
        // fill the `int` slot of the variant. Without two-pass loading `double`
