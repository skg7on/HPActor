...
/// Mark a function for addition at the beginning of the existing overload chain
/// instead of the end
struct prepend {};
...
struct argument_record {
    ...
    bool convert : 1;  ///< True if the argument is allowed to convert when
                       ///< loading
...
struct function_record {
    ...
    /// True if this function is to be inserted at the beginning of the overload
    /// resolution chain
    bool prepend : 1;
    ...
    /// Number of leading positional arguments, which are terminated by a
    /// py::args or py::kwargs argument or by a py::kw_only annotation.
    std::uint16_t nargs_pos = 0;

    /// Number of leading arguments (counted in `nargs`) that are
    /// positional-only
...
struct type_record {
    ...
    add_base(const std::type_info& base, void* (*caster)(void*)) {
...
inline void append_self_arg_if_needed(function_record* r) {
    if (r->is_method && r->args.empty()) {
        r->args.emplace_back("self", nullptr, handle(), /*convert=*/true,
                             /*none=*/false);
    }
}
...
template <> struct process_attribute<arg_v> : process_attribute_default<arg_v> {
    static void init(const arg_v& a, function_record* r) {
        if (r->is_method && r->args.empty()) {
            r->args.emplace_back("self", /*descr=*/nullptr, /*parent=*/handle(),
                                 /*convert=*/true, /*none=*/false);
        }
...
struct process_attribute<pos_only> : process_attribute_default<pos_only> {
    static void init(const pos_only&, function_record* r) {
        ...
        // It also can't follow a kw_only, but a static_assert in pybind11.h
        // checks that
    }
};
...
template <> struct process_attribute<custom_type_setup> {
...
/// Process a 'prepend' attribute, putting this at the beginning of the overload
/// chain
template <>
...
template <typename... Args> struct process_attributes {
    static void init(const Args&... args, function_record* r) {
        ...
        (void)expander{
            0,
            ((void)process_attribute<typename std::decay<Args>::type>::init(args, r),
             0)...};
    }
    ...
    static void postcall(function_call& call, handle fn_ret) {
        ...
        using expander = int[];
        (void)expander{0, (process_attribute<typename std::decay<Args>::type>::postcall(
                               call, fn_ret),
                           0)...};
    }
};
...
constexpr bool expected_num_args(size_t nargs, bool has_args, bool has_kwargs) {
    ...
    return named == 0 ||
           (self + named + size_t(has_args) + size_t(has_kwargs)) == nargs;
}
