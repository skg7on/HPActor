...
inline std::vector<ssize_t>
c_strides(const std::vector<ssize_t>& shape, ssize_t itemsize) {
...
// f_style`
inline std::vector<ssize_t>
f_strides(const std::vector<ssize_t>& shape, ssize_t itemsize) {
    auto ndim = shape.size();
    std::vector<ssize_t> strides(ndim, itemsize);
...
struct buffer_info {
    ...
    bool readonly = false; // flag to indicate if the underlying storage may be
                           // written to
    ...
    buffer_info(T* ptr, detail::any_container<ssize_t> shape_in,
        ...
        : buffer_info(private_ctr_tag(), ptr, sizeof(T),
                      ...
                      std::move(shape_in), std::move(strides_in), readonly) {}

    buffer_info(void* ptr, ssize_t itemsize, const std::string& format,
                ssize_t size, bool readonly = false)
    ...
    buffer_info(const T* ptr, ssize_t size, bool readonly = true)
        : buffer_info(const_cast<T*>(ptr), sizeof(T),
    ...
    explicit buffer_info(Py_buffer* view, bool ownview = true)
        : buffer_info(
              ...
              view->strides
                  ...
                  : detail::c_strides({view->shape, view->shape + view->ndim},
                                      view->itemsize),
              (view->readonly != 0)) {
    ...
    template <typename T> bool item_type_is_equivalent_to() const {
...
template <typename T, typename SFINAE> struct compare_buffer_info {
