...
typename make_caster<T>::template cast_op_type<T> cast_op(make_caster<T>& caster) {
    using result_t = typename make_caster<T>::template cast_op_type<T>; // See
                                                                        // PR
                                                                        // #4893
...
template <typename type> class type_caster<std::reference_wrapper<type>> {
  ...
  public:
    ...
    static handle cast(const std::reference_wrapper<type>& src,
        ...
        // It is definitely wrong to take ownership of this pointer, so mask
        // that rvp
    ...
    explicit operator std::reference_wrapper<type>() {
  ...
  public:                                                                                   \
    ...
    template <typename T_,                                                                  \
              ::pybind11::detail::enable_if_t<                                              \
                  std::is_same<type, ::pybind11::detail::remove_cv_t<T_>>::value, int> = 0> \
    static ::pybind11::handle cast(T_* src, ::pybind11::return_value_policy policy,         \
...
struct type_caster<T, enable_if_t<std::is_arithmetic<T>::value && !is_std_char_type<T>::value>> {
  ...
  public:
    bool load(handle src, bool convert) {
        ...
        // Check to see if the conversion is valid (integers should match
        // exactly) Signed/unsigned checks happen elsewhere
        if (py_err || (std::is_integral<T>::value && sizeof(py_type) != sizeof(T) &&
                       py_value != (py_type)(T)py_value)) {
            PyErr_Clear();
            if (py_err && convert && (PyNumber_Check(src.ptr()) != 0)) {
                auto tmp = reinterpret_steal<object>(
                    std::is_floating_point<T>::value ? PyNumber_Float(src.ptr())
    ...
    static
        typename std::enable_if<!std::is_floating_point<U>::value &&
                                    std::is_signed<U>::value && (sizeof(U) <= sizeof(long)),
                                handle>::type
        cast(U src, return_value_policy /* policy */, handle /* parent */) {
...
template <typename T> struct void_caster {
  public:
    ...
    static handle
    cast(T, return_value_policy /* policy */, handle /* parent */) {
        return none().release();
    }
...
template <> class type_caster<void> : public type_caster<void_type> {
  public:
    ...
    bool load(handle h, bool) {
        ...
        if (bases.size() == 1) { // Only allowing loading from a single-value
                                 // type
...
template <> class type_caster<bool> {
  public:
    ...
    static handle
    cast(bool src, return_value_policy /* policy */, handle /* parent */) {
...
template <typename StringType, bool IsView = false> struct string_caster {
    ...
    bool load(handle src, bool) {
        ...
        if (UTF_N == 8) {
            Py_ssize_t size = -1;
            const auto* buffer = reinterpret_cast<const CharT*>(
                PyUnicode_AsUTF8AndSize(load_src.ptr(), &size));
            if (!buffer) {
                PyErr_Clear();
                return false;
            }
        ...
        // If we're loading a string_view we need to keep the encoded Python
        // object alive:
        if (IsView) {
  ...
  private:
...
#else
        // PyPy segfaults when on PyUnicode_DecodeUTF16 (and possibly on
        // PyUnicode_DecodeUTF32 as well), so bypass the whole thing by just
    ...
    // When loading into a std::string or char*, accept a bytes/bytearray object
    // as-is (i.e. without any encoding/decoding attempt).  For other C++ char
    // sizes this is a no-op. which supports loading a unicode from a str,
...
struct type_caster<CharT, enable_if_t<is_std_char_type<CharT>::value>> {
  ...
  public:
    ...
    explicit operator CharT&() {
        ...
        // If we're in UTF-8 mode, we have two possible failures: one for a
        // unicode character that is too high, and one for multiple unicode
        // characters (caught later), so we need to figure out how long the
        // first encoded character is in bytes to distinguish between these two
        // errors.  We also allow want to allow unicode characters U+0080
        // through U+00FF, as those can fit into a single char value.
        if (StringCaster::UTF_N == 8 && str_len > 1 && str_len <= 4) {
            ...
            if (char0_bytes == str_len) {
                // If we have a 128-255 value, we can decode it into a single
                // char:
                if (char0_bytes == 2 && (v0 & 0xFC) == 0xC0) { // 0x110000xx
                                                               // 0x10xxxxxx
        ...
        // UTF-16 is much easier: we can only have a surrogate pair for values
        // above U+FFFF, thus a surrogate pair with total length 2 instantly
        // indicates a range error (but not a "your string was too long" error).
...
class tuple_caster {
  ...
  public:
    ...
    static constexpr auto name =
        const_name("tuple[") +
        ::pybind11::detail::concat(make_caster<Ts>::name...) + const_name("]");
...
template <> struct handle_type_name<object> {
    static constexpr auto name = const_name("object");
};
template <> struct handle_type_name<list> {
    static constexpr auto name = const_name("list");
};
template <> struct handle_type_name<dict> {
    static constexpr auto name = const_name("dict");
};
template <> struct handle_type_name<anyset> {
    static constexpr auto name = const_name("Union[set, frozenset]");
};
template <> struct handle_type_name<set> {
    static constexpr auto name = const_name("set");
};
template <> struct handle_type_name<frozenset> {
    static constexpr auto name = const_name("frozenset");
};
template <> struct handle_type_name<str> {
    static constexpr auto name = const_name("str");
};
template <> struct handle_type_name<tuple> {
    static constexpr auto name = const_name("tuple");
};
template <> struct handle_type_name<bool_> {
    static constexpr auto name = const_name("bool");
};
template <> struct handle_type_name<bytes> {
    static constexpr auto name = const_name(PYBIND11_BYTES_NAME);
};
template <> struct handle_type_name<buffer> {
    static constexpr auto name = const_name("Buffer");
};
template <> struct handle_type_name<int_> {
    static constexpr auto name = const_name("int");
};
template <> struct handle_type_name<iterable> {
    static constexpr auto name = const_name("Iterable");
};
template <> struct handle_type_name<iterator> {
    static constexpr auto name = const_name("Iterator");
};
template <> struct handle_type_name<float_> {
    static constexpr auto name = const_name("float");
};
template <> struct handle_type_name<function> {
    static constexpr auto name = const_name("Callable");
};
template <> struct handle_type_name<handle> {
    static constexpr auto name = handle_type_name<object>::name;
};
template <> struct handle_type_name<none> {
    static constexpr auto name = const_name("None");
};
template <> struct handle_type_name<sequence> {
    static constexpr auto name = const_name("Sequence");
};
template <> struct handle_type_name<bytearray> {
    static constexpr auto name = const_name("bytearray");
};
template <> struct handle_type_name<memoryview> {
    static constexpr auto name = const_name("memoryview");
};
template <> struct handle_type_name<slice> {
    static constexpr auto name = const_name("slice");
};
template <> struct handle_type_name<type> {
    static constexpr auto name = const_name("type");
};
template <> struct handle_type_name<capsule> {
    static constexpr auto name = const_name("capsule");
};
template <> struct handle_type_name<ellipsis> {
    static constexpr auto name = const_name("ellipsis");
};
template <> struct handle_type_name<weakref> {
...
template <> struct handle_type_name<obj_attr_accessor> {
    static constexpr auto name = const_name<obj_attr_accessor>();
};
template <> struct handle_type_name<str_attr_accessor> {
    static constexpr auto name = const_name<str_attr_accessor>();
};
template <> struct handle_type_name<item_accessor> {
    static constexpr auto name = const_name<item_accessor>();
};
template <> struct handle_type_name<sequence_accessor> {
    static constexpr auto name = const_name<sequence_accessor>();
};
template <> struct handle_type_name<list_accessor> {
    static constexpr auto name = const_name<list_accessor>();
};
template <> struct handle_type_name<tuple_accessor> {
...
template <typename type> struct pyobject_caster {
    ...
    // `type` may not be default constructible (e.g. frozenset, anyset).
    // Initializing `value` to a nil handle is safe since it will only be
    // accessed if `load` succeeds.
...
struct move_always<
    T, enable_if_t<all_of<
           move_is_plain_type<T>, negation<is_copy_constructible<T>>, is_move_constructible<T>,
...
struct move_if_unreferenced<
    T, enable_if_t<all_of<
           move_is_plain_type<T>, negation<move_always<T>>, is_move_constructible<T>,
           std::is_same<decltype(std::declval<make_caster<T>>().operator T&()), T&>>::value>>
    : std::true_type {};
template <typename T>
...
type_caster<T, SFINAE>&
load_type(type_caster<T, SFINAE>& conv, const handle& handle) {
...
// pytype -> C++ type
template <typename T, detail::enable_if_t<!detail::is_pyobject<T>::value &&
...
detail::enable_if_t<!detail::move_never<T>::value, T> move(object&& obj) {
...
#if !defined(PYBIND11_DETAILED_ERROR_MESSAGES)
        throw cast_error(
            "Unable to cast Python " +
            str(type::handle_of(obj)).cast<std::string>() +
    ...
    // Move into a temporary and return that, because the reference may be a
    // local value of `conv`
    T ret = std::move(detail::load_type<T>(obj).operator T&());
    return ret;
}

// Calling cast() on an rvalue calls pybind11::cast with the object rvalue,
// which does:
...
template <> inline void object::cast() const& {
    return;
}
template <> inline void object::cast() && {
    return;
}
...
template <typename T>
enable_if_t<!cast_is_temporary_value_reference<T>::value, T>
cast_ref(object&&, override_unused&) {
...
enable_if_t<detail::none_of<cast_is_temporary_value_reference<T>,
                            detail::is_same_ignoring_cvref<T, PyObject*>, std::is_void<T>>::value,
            T>
cast_safe(object&& o) {
...
struct arg {
    /// Constructs an argument with the name of the argument; if null or
    /// omitted, this is a positional argument.
    ...
    /// Indicates that the argument should/shouldn't allow None (e.g. for
    /// nullable pointer args)
    ...
    bool flag_none : 1; ///< If set (the default), allow None to be passed to
                        ///< this argument
};
...
/// \ingroup annotations
/// Annotation indicating that all following arguments are keyword-only; the is
/// the equivalent of an unnamed '*' argument
struct kw_only {};

/// \ingroup annotations
/// Annotation indicating that all previous arguments are positional-only; the
/// is the equivalent of an unnamed '/' argument (in Python 3.8)
struct pos_only {};
...
struct function_call {
    function_call(const function_record& f, handle p); // Implementation in
                                                       // attr.h
    ...
    /// Extra references for the optional `py::args` and/or `py::kwargs`
    /// arguments (which, if present, are also in `args` but without a
    /// reference).
...
template <typename... Args> class argument_loader {
    ...
    static constexpr auto kwargs_pos =
        constexpr_last<argument_is_kwargs, Args...>();
...
#ifdef __cpp_fold_expressions
        if ((... ||
             !std::get<Is>(argcasters).load(call.args[Is], call.args_convert[Is]))) {
            return false;
        }
#else
        for (bool r :
...
/// `PYBIND11_OVERRIDE(PYBIND11_TYPE(ReturnType<A, B>), PYBIND11_TYPE(Parent<C,
/// D>), f, arg)`
