/*
    pybind11/detail/type_caster_base.h (originally first part of
   pybind11/cast.h)
...
class loader_life_support {
  private:
    ...
    static loader_life_support* get_stack_top() {
        return static_cast<loader_life_support*>(
            PYBIND11_TLS_GET_VALUE(get_stack_tls_key()));
    }
  ...
  public:
    ...
    loader_life_support() : parent{get_stack_top()} {
        set_stack_top(this);
    }
    ...
    PYBIND11_NOINLINE static void add_patient(handle h) {
        ...
        if (!frame) {
            // NOTE: It would be nice to include the stack frames here, as this
            // indicates use of pybind11::cast<> outside the normal call
            // framework, finding such a location is challenging. Developers
            // could consider printing out stack frame addresses here using
            // something like __builtin_frame_address(0)
...
// Band-aid workaround to fix a subtle but serious bug in a minimalistic
// fashion. See PR #4762.
...
all_type_info_populate(PyTypeObject* t, std::vector<type_info*>& bases) {
    ...
    for (size_t i = 0; i < check.size(); i++) {
        ...
        if (it != type_dict.end()) {
            // We found a cache entry for it, so it's either pybind-registered
            // or has pre-computed pybind bases, but we have to make sure we
            // haven't already seen the type(s) before: we want to follow
            // Python/virtual C++ rules that there should only be one instance
            // of a common base.
            for (auto* tinfo : it->second) {
                // NB: Could use a second set here, rather than doing a linear
                // search, but since having a large number of immediate
                // pybind11-registered types seems fairly unlikely, that
            ...
            // It's some python type, so keep follow its bases classes to look
            // for one or more registered types
            if (i + 1 == check.size()) {
                // When we're at the end, we can pop off the current element to
                // avoid growing `check` when adding just one base (which is
                // typical--i.e. when there is no multiple inheritance)
                check.pop_back();
                i--;
            }
...
PYBIND11_NOINLINE detail::type_info*
...
// Searches the inheritance graph for a registered Python instance, using
// all_type_info().
PYBIND11_NOINLINE handle
    ...
    return with_instance_map(src, [&](instance_map& instances) {
        ...
        for (auto it_i = it_instances.first; it_i != it_instances.second; ++it_i) {
            for (auto* instance_type :
                 detail::all_type_info(Py_TYPE(it_i->second))) {
                if (instance_type &&
...
struct values_and_holders {
  ...
  public:
    ...
    struct iterator {
      private:
        ...
        iterator(instance* inst, const type_vec* tinfo)
            ...
            if (inst != nullptr) {
                assert(!types->empty());
                curr = value_and_holder(inst /* instance */,
                                        (*types)[0] /* type info */, 0, /* vpos:
                                                                           (non-simple
                                                                           types
                                                                           only):
                                                                           the
    ...
    iterator begin() {
        return iterator(inst, &tinfo);
    }
    iterator end() {
        return iterator(tinfo.size());
    }
 ...
 */
PYBIND11_NOINLINE value_and_holder instance::get_value_and_holder(
...
PYBIND11_NOINLINE void instance::allocate_layout() {
    ...
    // Simple path: no python-side multiple inheritance, and a small-enough
    // holder
    if (simple_layout) {
        ...
        // value pointer, [hN] is the (uninitialized) holder instance for value
        // N, and [bb...] is a set of bool values that tracks whether each
        // associated holder has been initialized.  Each [block] is padded, if
        // necessary, to an integer multiple of sizeof(void *).
        size_t space = 0;
        for (auto* t : tinfo) {
        ...
        // allocation functions: Python is using pymalloc, which is designed to
        // be efficient for small allocations like the one we're doing here; for
        // larger allocations they are just wrappers around malloc.
        // TODO: is this still true for pure Python 3.6?
        nonsimple.values_and_holders = (void**)PyMem_Calloc(space, sizeof(void*));
        if (!nonsimple.values_and_holders) {
            throw std::bad_alloc();
        }
        nonsimple.status = reinterpret_cast<std::uint8_t*>(
...
class type_caster_generic {
  public:
    ...
    PYBIND11_NOINLINE bool try_load_foreign_module_local(handle src) {
        ...
        // Only consider this foreign loader if actually foreign and is a loader
        // of the correct cpp type
    ...
    // dispatch the relevant bits of code between here and
    // copyable_holder_caster where the two classes need different logic
    // (without having to resort to virtual inheritance).
    ...
    PYBIND11_NOINLINE bool load_impl(handle src, bool convert) {
        ...
        if (PyType_IsSubtype(srctype, typeinfo->type)) {
            ...
            // inherits from just one simple (no MI) pybind11 class, or is an
            // exact match, so the C++ instance is of the right type and we can
            // use reinterpret_cast. (This is essentially the same as case 2b,
            // but because not using multiple inheritance is extremely common,
            // we handle it specially to avoid the loop iterator and type
            ...
            // Case 2b: the python type inherits from multiple C++ bases.  Check
            // the bases to see if we can find an exact match (or, for a simple
            // C++ type, an inherited match); if so, we can safely
            // reinterpret_cast to the relevant pointer.
            if (bases.size() > 1) {
                for (auto* base : bases) {
                    if (no_cpp_mi ? PyType_IsSubtype(base->type, typeinfo->type)
                                  : base->type == typeinfo->type) {
                        this_.load_value(reinterpret_cast<instance*>(src.ptr())
                                             ->get_value_and_holder(base));
                        return true;
                    }
                }
            }

            // Case 2c: C++ multiple inheritance is involved and we couldn't
            // find an exact type match in the registered bases, above, so try
            // implicit casting (needed for proper C++ casting when MI is
            // involved).
        ...
        if (convert) {
            for (const auto& converter : typeinfo->implicit_conversions) {
                auto temp = reinterpret_steal<object>(
                    converter(src.ptr(), typeinfo->type));
    ...
    // the error and returns a pair with .second = nullptr.  (p.first = nullptr
    // is not an error: it becomes None).
...
inline object cpp_conduit_method(handle self, const bytes& pybind11_platform_abi_id,
    ...
    if (std::strcmp(cpp_type_info_capsule.name(), typeid(std::type_info).name()) !=
        0) {
        return none();
    }
    if (cpp_str(pointer_kind) != "raw_pointer_ephemeral") {
        throw std::runtime_error("Invalid pointer_kind: \"" +
                                 std::string(pointer_kind) + "\"");
    }
    const auto* cpp_type_info =
        cpp_type_info_capsule.get_pointer<const std::type_info>();
...
struct container_mapped_type_traits<
    Container,
    typename std::enable_if<std::is_same<typename Container::mapped_type, Container>::value>::type> {
...
struct container_value_type_traits<
    Container,
    typename std::enable_if<std::is_same<typename Container::value_type, Container>::value>::type> {
...
/*
 * For pairs - only as value type of a map -, the first type should remove the
 * `const`. Also, if the map is recursive, then the recursive checking should
 * consider the first type only.
 */
...
template <typename Container>
struct impl_recursive_container_traits<
    Container, typename std::enable_if<container_value_type_traits<Container>::has_value_type>::type> {
    static constexpr bool is_recursive =
        container_mapped_type_traits<Container>::has_recursive_mapped_type ||
        container_value_type_traits<Container>::has_recursive_value_type;
    /*
     * This member dictates which type Pybind11 should check recursively in
     * traits such as `is_move_constructible`, `is_copy_constructible`,
     * `is_move_assignable`, ... Direct access to `value_type` should be
     * avoided:
...
/*
 * This trait defines the `type_to_check_recursively` which is needed to
 * properly handle recursively defined traits such as `is_move_constructible`
 * without going into an infinite recursion. Should be used instead of directly
 * accessing the `value_type`. It cancels the recursion by returning the
 * `recursive_bottom` tag.
 *
 * The default definition of `type_to_check_recursively` is as follows:
 *
 * 1. By default, it is `recursive_bottom`, so that the recursion is canceled.
 * 2. If the type is non-recursive and defines a `value_type`, then the
 * `value_type` is used. If the `value_type` is a pair and a `mapped_type` is
 * defined, then the `const` is removed from the first type.
 * 3. If the type is recursive and `value_type` is not a pair, then
 * `recursive_bottom` is returned.
 * 4. If the type is recursive and `value_type` is a pair and a `mapped_type` is
 * defined, then `const` is removed from the first type and the first type is
 * returned.
 *
...
struct is_move_constructible
    : all_of<std::is_move_constructible<T>,
             is_move_constructible<typename recursive_container_traits<T>::type_to_check_recursively>> {
};

template <> struct is_move_constructible<recursive_bottom> : std::true_type {};

// Likewise for std::pair
// (after C++17 it is mandatory that the move constructor not exist when the two
// types aren't themselves move constructible, but this can not be relied upon
// when T1 or T2 are themselves containers).
...
struct is_copy_constructible
    : all_of<std::is_copy_constructible<T>,
             is_copy_constructible<typename recursive_container_traits<T>::type_to_check_recursively>> {
};

template <> struct is_copy_constructible<recursive_bottom> : std::true_type {};

// Likewise for std::pair
// (after C++17 it is mandatory that the copy constructor not exist when the two
// types aren't themselves copy constructible, but this can not be relied upon
// when T1 or T2 are themselves containers).
template <typename T1, typename T2>
struct is_copy_constructible<std::pair<T1, T2>>
    : all_of<is_copy_constructible<T1>, is_copy_constructible<T2>> {};

// The same problems arise with std::is_copy_assignable, so we use the same
// workaround.
template <typename T>
...
template <> struct is_copy_assignable<recursive_bottom> : std::true_type {};
...
//
// polymorphic_type_hook_base allows users to specialize polymorphic_type_hook
// with std::enable_if. User provided specializations will always have higher
// priority than the default implementation and specialization provided in
...
template <typename type> class type_caster_base : public type_caster_generic {
  ...
  public:
    ...
    // specializing polymorphic_type_hook). If the instance isn't derived,
    ...
    static std::pair<const void*, const type_info*> src_and_type(const itype* src) {
        ...
        if (instance_type && !same_type(cast_type, *instance_type)) {
            ...
            // available. In the typical case where itype is polymorphic, we get
            // the correct derived pointer (which may be != base pointer) by a
            // dynamic_cast to most derived type. If itype is not polymorphic,
            // we won't get here except via a user-provided specialization of
            // polymorphic_type_hook, and the user has promised that no
            // this-pointer adjustment is required in that case, so it's OK to
            // use static_cast.
  ...
  protected:
    ...
    /* Only enabled when the types are {copy,move}-constructible *and* when the
       type does not have a private operator new implementation. A comma
       operator is used in the decltype argument to apply SFINAE to the public
