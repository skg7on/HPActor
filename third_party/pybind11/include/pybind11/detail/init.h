...
template <> class type_caster<value_and_holder> {
  public:
    ...
    explicit operator value_and_holder&() {
        return *value;
    }
...
// false)
template <typename /*Class*/> constexpr bool is_alias(void*) {
    return false;
}

// Constructs and returns a new object; if the given arguments don't map to a
// constructor, we fall back to brace aggregate initialization so that for
...
void construct(value_and_holder& v_h, Cpp<Class>* ptr, bool need_alias) {
    ...
    if (Class::has_alias && need_alias && !is_alias<Class>(ptr)) {
        // We're going to try to construct an alias by moving the cpp type.
        // Whether or not that succeeds, we still need to destroy the original
        // cpp pointer (either the moved away leftover, if the alias
        // construction works, or the value itself if we throw an error), but we
        // can't just call `delete ptr`: it might have a special deleter, or
        // might be shared_from_this.  So we construct a holder around it as if
        // it was a normal instance, then steal the holder away into a local
        // variable; thus the holder and destruction happens when we leave the
        // C++ scope, and the holder class gets to handle the destruction
        // however it likes.
        v_h.value_ptr() = ptr;
        v_h.set_instance_registered(true); // Trick to prevent init_instance
                                           // from registering it
        // DANGER ZONE BEGIN: exceptions will leave v_h in an invalid state.
        v_h.type->init_instance(v_h.inst, nullptr); // Set up the holder
        Holder<Class> temp_holder(std::move(v_h.holder<Holder<Class>>())); // Steal
                                                                           // the
                                                                           // holder
        v_h.type->dealloc(v_h); // Destroys the moved-out holder remains, resets
                                // value ptr to null
...
void construct(value_and_holder& v_h, Cpp<Class>&& result, bool need_alias) {
    PYBIND11_WORKAROUND_INCORRECT_MSVC_C4100(need_alias);
    static_assert(
        is_move_constructible<Cpp<Class>>::value,
...
template <typename... Args> struct constructor {
    ...
    template <typename Class, typename... Extra,
              enable_if_t<Class::has_alias && std::is_constructible<Cpp<Class>, Args...>::value, int> = 0>
    static void execute(Class& cl, const Extra&... extra) {
        cl.def(
            "__init__",
            [](value_and_holder& v_h, Args... args) {
...
struct factory<Func, void_type (*)(), Return(Args...)> {
    ...
    // an alias type and an alias instance is needed (i.e. because the final
    // type is a Python class inheriting from the C++ type) the returned value
    // needs to either already be an alias instance, or the alias needs to be
    // constructible from a `Class &&` argument.
    template <typename Class, typename... Extra>
    ...
    void execute(Class& cl, const Extra&... extra) && {
        ...
        cl.def(
            ...
            (value_and_holder& v_h, CArgs... args) {
                if (Py_TYPE(v_h.inst) == v_h.type->type) {
                    // If the instance type equals the registered type we don't
                    // have inheritance, so don't need the alias and can
                    // construct using the class function:
                    construct<Class>(
                        v_h, class_func(std::forward<CArgs>(args)...), false);
                } else {
                    construct<Class>(
                        v_h, alias_func(std::forward<CArgs>(args)...), true);
                }
            },
            is_new_style_constructor(), extra...);
    }
...
void setstate(value_and_holder& v_h, std::pair<T, O>&& result, bool need_alias) {
    ...
    if (PyDict_Check(d.ptr()) && PyDict_Size(d.ptr()) == 0) {
        // Skipping setattr below, to not force use of py::dynamic_attr() for
        // Class unnecessarily. See PR #2972 for details.
        return;
    }
