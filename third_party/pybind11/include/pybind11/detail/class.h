...
#else
// In PyPy, we still set __qualname__ so that we can produce reliable function
// type signatures; in CPython this macro expands to nothing:
#    define PYBIND11_SET_OLDPY_QUALNAME(obj, nameobj)                          \
...
#else
    auto module_name =
        handle((PyObject*)type).attr("__module__").cast<std::string>();
...
pybind11_meta_setattro(PyObject* obj, PyObject* name, PyObject* value) {
    // Use `_PyType_Lookup()` instead of `PyObject_GetAttr()` in order to get
    // the raw descriptor (`property`) instead of calling `tp_descr_get`
    // (`property.__get__()`).
    ...
    //   2. `Type.static_prop = other_static_prop` --> setattro:  replace
    //   existing `static_prop`
...
inline void
traverse_offset_bases(void* valueptr, const detail::type_info* tinfo, instance* self,
...
inline bool register_instance_impl(void* ptr, instance* self) {
    with_instance_map(
        ptr, [&](instance_map& instances) { instances.emplace(ptr, self); });
...
inline void
register_instance(instance* self, void* valptr, const type_info* tinfo) {
...
inline bool
deregister_instance(instance* self, void* valptr, const type_info* tinfo) {
...
inline PyObject* make_new_instance(PyTypeObject* type) {
    ...
    // the first inherited object is a plain Python type (i.e. not derived from
...
extern "C" inline int pybind11_object_init(PyObject* self, PyObject*, PyObject*) {
    ...
    std::string msg =
        get_fully_qualified_tp_name(type) + ": No constructor defined!";
...
inline void clear_instance(PyObject* self) {
    ...
    for (auto& v_h : values_and_holders(instance)) {
        if (v_h) {
            // We have to deregister before we call dealloc because, for virtual
            // MI types, we still need to be able to get the parent pointers.
...
extern "C" inline void pybind11_object_dealloc(PyObject* self) {
    ...
    // called as part of a derived type's dealloc, in which case we're not
    // allowed to decref the type here. For cross-module compatibility, we
    // shouldn't compare directly with `pybind11_object_dealloc`, but with the
    // common one stashed in internals.
...
/// `__dict__`.
extern "C" inline int
pybind11_traverse(PyObject* self, visitproc visit, void* arg) {
...
inline void enable_dynamic_attributes(PyHeapTypeObject* heap_type) {
    ...
    type->tp_basicsize += (ssize_t)sizeof(PyObject*); // and allocate enough
                                                      // space for it
#else
...
extern "C" inline int
pybind11_getbuffer(PyObject* obj, Py_buffer* view, int flags) {
