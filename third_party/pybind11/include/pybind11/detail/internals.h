...
// favor of the new Thread Specific Storage (TSS) API. Avoid unnecessary
// allocation of `Py_tss_t`, since we cannot use `Py_LIMITED_API` anyway.
...
#    define PYBIND11_TLS_KEY_CREATE(var)                                       \
        (((var) = PyThread_tss_alloc()) != nullptr &&                          \
         (PyThread_tss_create((var)) == 0))
#    define PYBIND11_TLS_GET_VALUE(key) PyThread_tss_get((key))
#    define PYBIND11_TLS_REPLACE_VALUE(key, value)                             \
        PyThread_tss_set((key), (value))
#    define PYBIND11_TLS_DELETE_VALUE(key) PyThread_tss_set((key), nullptr)
...
struct override_hash {
    ...
    operator()(const std::pair<const PyObject*, const char*>& v) const {
        ...
        value ^= std::hash<const void*>()(v.second) + 0x9e3779b9 +
                 (value << 6) + (value >> 2);
        return value;
    }
};
...
// Instance map shards are used to reduce mutex contention in free-threaded
// Python.
struct instance_map_shard {
    instance_map registered_instances;
    pymutex mutex;
    // alignas(64) would be better, but causes compile errors in macOS
    // before 10.14 (see #5200)
...
struct internals {
...
#ifdef Py_GIL_DISABLED
    std::unique_ptr<instance_map_shard[]> instance_shards; // void * ->
                                                           // instance*
    ...
    std::unordered_map<std::string, void*> shared_data; // Custom data to be
                                                        // shared across
                                                        // extensions
#if PYBIND11_INTERNALS_VERSION == 4
    std::vector<PyObject*> unused_loader_patient_stack_remove_at_v5;
#endif
    std::forward_list<std::string> static_strings; // Stores the std::strings
                                                   // backing detail::c_str()
...
#if PYBIND11_INTERNALS_VERSION > 4
    // Note that we have to use a std::string to allocate memory to ensure a
    // unique address We want unique addresses since we use pointer equality to
    // compare function records
    std::string function_record_capsule_name =
    ...
    ~internals() {
        ...
        // This destructor is called *after* Py_Finalize() in
        // finalize_interpreter(). That *SHOULD BE* fine. The following details
        // what happens when PyThread_tss_free is called. PYBIND11_TLS_FREE is
        // PyThread_tss_free on python 3.7+. On older python, it does nothing.
        // PyThread_tss_free calls PyThread_tss_delete and PyMem_RawFree.
        // PyThread_tss_delete just calls TlsFree (on Windows) or
        // pthread_key_delete (on *NIX). Neither of those have anything to do
        // with CPython internals. PyMem_RawFree *requires* that the `tstate` be
        // allocated with the CPython allocator.
...
struct type_info {
     ...
     * A type can be simple even if it has non-simple ancestors as long as it
     * has no descendants.
     */
    bool simple_type : 1;
...
#        define PYBIND11_BUILD_ABI                                             \
            "_cxxabi" PYBIND11_TOSTRING(__GXX_ABI_VERSION)
...
#define PYBIND11_PLATFORM_ABI_ID                                               \
    PYBIND11_INTERNALS_KIND PYBIND11_COMPILER_TYPE PYBIND11_STDLIB PYBIND11_BUILD_ABI PYBIND11_BUILD_TYPE
...
bool handle_nested_exception(const T& exc, const std::exception_ptr& p) {
    if (const auto* nep =
            dynamic_cast<const std::nested_exception*>(std::addressof(exc))) {
...
inline object get_python_state_dict() {
    object state_dict;
#if PYBIND11_INTERNALS_VERSION <= 4 || PY_VERSION_HEX < 0x03080000 ||          \
...
#    endif
    if (istate) {
        state_dict =
            reinterpret_borrow<object>(PyInterpreterState_GetDict(istate));
    }
#endif
    if (!state_dict) {
...
PYBIND11_NOINLINE internals& get_internals() {
    ...
    // Cannot use py::gil_scoped_acquire here since that constructor calls
    // get_internals.
    struct gil_scoped_acquire_local {
        gil_scoped_acquire_local() : state(PyGILState_Ensure()) {}
        gil_scoped_acquire_local(const gil_scoped_acquire_local&) = delete;
        gil_scoped_acquire_local&
        operator=(const gil_scoped_acquire_local&) = delete;
        ~gil_scoped_acquire_local() {
    ...
    if (internals_pp && *internals_pp) {
        // We loaded the internals through `state_dict`, which means that our
        // `error_already_set` and `builtin_exception` may be different local
        // classes than the ones set up in the initial exception translator,
        // below, so add another for our local exception classes.
        //
        // libstdc++ doesn't require this (types there are identified only by
        // name) libc++ with CPython doesn't require this (types are explicitly
        // exported) libc++ with PyPy still need it, awaiting further
        // investigation
#if !defined(__GLIBCXX__)
...
#ifdef Py_GIL_DISABLED
        // Scale proportional to the number of cores. 2x is a heuristic to
        // reduce contention.
        auto num_shards = static_cast<size_t>(
            round_up_to_next_pow2(2 * std::thread::hardware_concurrency()));
        if (num_shards == 0) {
            num_shards = 1;
        }
...
// the internals struct (above) is shared between all the modules.
// local_internals are only for a single module. Any changes made to internals
// may require an update to PYBIND11_INTERNALS_VERSION, breaking backwards
// compatibility. local_internals is, by design, restricted to a single module.
// Whether a module has local internals or not should not impact any other
// modules, because the only things accessing the local internals is the module
// that contains them.
struct local_internals {
    ...
    // the `internals` struct directly.  Instead, we store it in `shared_data`
    // and cache a copy in `local_internals`.  If we allocated a separate TLS
    // key for each instance of `local_internals`, we could end up allocating
    // hundreds of TLS keys if hundreds of different pybind11 modules are loaded
    ...
    struct shared_loader_life_support_data {
        ...
        // We can't help but leak the TLS key, because Python never unloads
        // extension modules.
    };
...
inline local_internals& get_local_internals() {
    // Current static can be created in the interpreter finalization routine. If
    // the later will be destroyed in another static variable destructor,
    // creation of this static there will cause static deinitialization fiasco.
    // In order to avoid it we avoid destruction of the local_internals static.
    // One can read more about the problem and current solution here:
...
#    define PYBIND11_LOCK_INTERNALS(internals)                                 \
        std::unique_lock<pymutex> lock((internals).mutex)
#else
#    define PYBIND11_LOCK_INTERNALS(internals)
#endif
...
// Returns the number of registered instances for testing purposes.  The result
// may not be consistent if other threads are registering or unregistering
...
// Determine whether or not the following capsule contains a pybind11 function
// record. Note that we use `internals` to make sure that only ABI compatible
// records are touched.
//
// This check is currently used in two places:
// - An important optimization in functional.h to avoid overhead in C++ ->
// Python -> C++
