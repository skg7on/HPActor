...
// Similar to Python's convention:
// https://docs.python.org/3/c-api/apiabiversion.html Additional convention: 0xD
// = dev
...
// PYBIND11_NAMESPACE_BEGIN/PYBIND11_NAMESPACE_END pair of macros. Therefore
// manual PYBIND11_WARNING_PUSH/PYBIND11_WARNING_POP are usually only needed in
// `#include` sections.
//
// If you find you need to suppress a warning, please try to make the
// suppression as local as possible using these macros. Please also be sure to
// push/pop with the pybind11 macros. Please only use compiler specifics if you
// need to check specific versions, e.g. Apple Clang vs. vanilla Clang.
#if defined(_MSC_VER)
...
#    define PYBIND11_WARNING_DISABLE_MSVC(name)                                \
        PYBIND11_PRAGMA(warning(disable : name))
#else
#    define PYBIND11_WARNING_DISABLE_MSVC(name)
#endif
...
#    define PYBIND11_WARNING_DISABLE_CLANG(name)                               \
        PYBIND11_PRAGMA(clang diagnostic ignored name)
#else
#    define PYBIND11_WARNING_DISABLE_CLANG(name)
#endif
...
#    define PYBIND11_WARNING_DISABLE_GCC(name)                                 \
        PYBIND11_PRAGMA(GCC diagnostic ignored name)
#else
#    define PYBIND11_WARNING_DISABLE_GCC(name)
#endif
...
#        define PYBIND11_NAMESPACE                                             \
            pybind11 __attribute__((visibility("hidden")))
#    else
 ...
 */
#    pragma warning disable 2196 // warning #2196: routine is both "inline" and
                                 // "noinline"
...
// `-Wno-attributes`. However, the measured shared-library size saving when
// using noinline are only 1.7% for CUDA, -0.2% for GCC7, and 0.0% for GCC8
// (using -DCMAKE_BUILD_TYPE=MinSizeRel, the default under pybind11/tests).
...
#if defined(PYBIND11_NOINLINE_DISABLED) // Option for maximum portability and
                                        // experimentation.
...
// For unknown reasons all PYBIND11_DEPRECATED member trigger a warning when
// declared whether it is used or not
...
// requirement: https://docs.python.org/3/c-api/intro.html#include-files See
// https://github.com/pybind/pybind11/pull/3497 for full context.
#        include <yvals.h>
...
// Must be after including <version> or one of the other headers specified by
// the standard
...
interpreter imports a plugin library. Please create a `module_` in the function
...
/** \rst
    This macro creates the entry point that will be invoked when the Python
interpreter imports an extension module. The module name is given as the first
argument and it should not be in quotes. The second macro argument defines a
variable of type `py::module_` which can be used to initialize the module.

    The entry point is marked as "maybe unused" to aid dead-code detection
analysis: since the entry point is typically only looked up at runtime and not
referenced during translation, it would otherwise appear as unused ("dead")
code.
    ...
    The third macro argument is optional (available since 2.13.0), and can be
used to mark the extension module as safe to run without the GIL under a
free-threaded CPython interpreter. Passing this argument has no effect on other
interpreters.
...
#define PYBIND11_MODULE(name, variable, ...)                                   \
    ...
    PYBIND11_PLUGIN_IMPL(name) {                                               \
        ...
        auto m = ::pybind11::module_::create_extension_module(                 \
            ...
            &PYBIND11_CONCAT(pybind11_module_def_, name), ##__VA_ARGS__);      \
        try {                                                                  \
...
template <typename IntType> inline ssize_t ssize_t_cast(const IntType& val) {
...
struct instance {
     ...
     * is set with a pointer and the holder object governing that pointer, i.e.
     * [val1*][holder].  This layout is applied whenever there is no python-side
     * multiple inheritance of bound C++ types *and* the type's holder will fit
     * in the default space (which is large enough to hold either a
     * std::unique_ptr or std::shared_ptr).
     *
     * Non-simple layout applies when using custom holders that require more
     * space than `shared_ptr` (which is typically the size of two pointers), or
     * when multiple inheritance is used on the python side.  Non-simple layout
     * allocates the required amount of memory to have multiple bound C++
     * classes as parents.  Under this layout, `nonsimple.values_and_holders` is
     * set to a pointer to allocated space of the required space to hold a
     * sequence of value pointers and holders followed `status`, a set of bit
     * flags (1 byte each), i.e. [val1*][holder1][val2*][holder2]...[bb...]
     * where each [block] is rounded up to a multiple of `sizeof(void *)`.
     * `nonsimple.status` is, for convenience, a pointer to the beginning of the
     * [bb...] block (but not independently allocated).
     *
    ...
    /// Returns the value_and_holder wrapper for the given type (or the first,
    /// if `find_type` omitted).  Returns a default-constructed (with `.inst =
...
constexpr int last(int /*i*/, int result) {
    return result;
}
template <typename T, typename... Ts>
...
/// that a Derived pointer can be converted to a Base pointer) For unions,
/// `is_base_of<T, T>::value` is False, so we need to check `is_same` as well.
template <typename Base, typename Derived>
...
// Intel compiler produces an internal error on this fold expression (tested
// with ICC 19.0.2)
#    define PYBIND11_EXPAND_SIDE_EFFECTS(PATTERN) (((PATTERN), void()), ...)
#else
using expand_side_effects = bool[];
#    define PYBIND11_EXPAND_SIDE_EFFECTS(PATTERN)                              \
        (void)pybind11::detail::expand_side_effects {                          \
...
PYBIND11_RUNTIME_EXCEPTION(cast_error, PyExc_RuntimeError) /// Thrown when
                                                           /// pybind11::cast or
                                                           /// handle::call fail
                                                           /// due to a type
                                                           /// casting error
PYBIND11_RUNTIME_EXCEPTION(reference_cast_error, PyExc_RuntimeError) /// Used
                                                                     /// internally
...
struct format_descriptor<
    T, detail::enable_if_t<detail::is_same_ignoring_cvref<T, PyObject*>::value>> {
...
template <typename... Args> struct overload_cast_impl {
    ...
    constexpr auto
    operator()(Return (Class::*pmf)(Args...), std::false_type = {}) const noexcept
        -> decltype(pmf) {
        return pmf;
    }
    ...
    template <typename Return, typename Class>
    constexpr auto
    operator()(Return (Class::*pmf)(Args...) const, std::true_type) const noexcept
        -> decltype(pmf) {
        return pmf;
    }
};
...
#if !defined(PYBIND11_CPP14) // no overload_cast: providing something that
                             // static_assert-fails:
...
// Pre C++17, this code path exploits undefined behavior, but is known to work
// on many platforms. Use at your own risk! See also
// https://en.cppreference.com/w/cpp/memory/enable_shared_from_this, and in
// particular the `std::shared_ptr<Good> gp1 = not_so_good.getptr();` and
// `try`-`catch` parts of the example.
...
#    define PYBIND11_WORKAROUND_INCORRECT_GCC_UNUSED_BUT_SET_PARAMETER(...)
    ...
    (defined(__apple_build_version__) /* AppleClang 13.0.0.13000029 was the    \
                                         only data point available. */                                                                 \
     || (__clang_major__ >= 7 && __clang_major__ <= 12) /* Clang 3, 5, 13, 14, \
                                                           15 do not generate  \
                                                           the warning. */     \
    )
#    define PYBIND11_DETECTED_CLANG_WITH_MISLEADING_CALL_STD_MOVE_EXPLICITLY_WARNING
// Example:
// tests/test_kwargs_and_defaults.cpp:46:68: error: local variable 'args' will
// be copied despite being returned by name [-Werror,-Wreturn-std-move]
//     m.def("args_function", [](py::args args) -> py::tuple { return args; });
//                                                                    ^~~~
// test_kwargs_and_defaults.cpp:46:68: note: call 'std::move' explicitly to
// avoid copying
//     m.def("args_function", [](py::args args) -> py::tuple { return args; });
//                                                                    ^~~~
//                                                                    std::move(args)
...
// Pybind offers detailed error messages by default for all builts that are
// debug (through the negation of NDEBUG). This can also be manually enabled by
// users, for any builds, through defining PYBIND11_DETAILED_ERROR_MESSAGES.
// This information is primarily useful for those who are writing (as opposed to
// merely using) libraries that use pybind11.
