...
constexpr descr<N1 + N2, Ts1..., Ts2...>
operator+(const descr<N1, Ts1...>& a, const descr<N2, Ts2...>& b) {
    return plus_impl(a, b, make_index_sequence<N1>(), make_index_sequence<N2>());
}

template <size_t N> constexpr descr<N - 1> const_name(char const (&text)[N]) {
...
const_name(char const (&text1)[N1], char const (&)[N2]) {
    return const_name(text1);
}
template <bool B, size_t N1, size_t N2>
constexpr enable_if_t<!B, descr<N2 - 1>>
const_name(char const (&)[N1], char const (&text2)[N2]) {
...
auto constexpr const_name()
    -> remove_cv_t<decltype(int_to_str<Size / 10, Size % 10>::digits)> {
    return int_to_str<Size / 10, Size % 10>::digits;
}

template <typename Type> constexpr descr<1, Type> const_name() {
    return {'%'};
}
...
// (The const_name code is repeated to avoid introducing a "_" #define
// ourselves.)
#ifndef _
...
template <bool B, size_t N1, size_t N2>
constexpr enable_if_t<B, descr<N1 - 1>>
_(char const (&text1)[N1], char const (&text2)[N2]) {
...
template <typename Type> constexpr descr<1, Type> _() {
...
template <size_t N1, size_t N2, typename... Ts1, typename... Ts2>
constexpr descr<N1 + N2 + 2, Ts1..., Ts2...>
operator,(const descr<N1, Ts1...>& a, const descr<N2, Ts2...>& b) {
