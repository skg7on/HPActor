...
//
//   static auto imported_obj = py::module_::import("module_name"); // BAD, DO
//   NOT USE!
//
// which has two serious issues:
//
//     1. Py_DECREF() calls potentially after the Python interpreter was
//     finalized already, and
//     2. deadlocks in multi-threaded processes (because of missing lock
//     ordering).
//
// The following alternative avoids both problems:
//
//   PYBIND11_CONSTINIT static py::gil_safe_call_once_and_store<py::object>
//   storage; auto &imported_obj = storage // Do NOT make this `static`!
  ...
  public:
    ...
    gil_safe_call_once_and_store& call_once_and_store_result(Callable&& fn) {
        if (!is_initialized_) { // This read is guarded by the GIL.
            // Multiple threads may enter here, because the GIL is released in
            // the next line and CPython API calls in the `fn()` call below may
            // release and reacquire the GIL.
            gil_scoped_release gil_rel; // Needed to establish lock ordering.
            std::call_once(once_flag_, [&] {
                // Only one thread will ever enter here.
                gil_scoped_acquire gil_acq;
                ::new (storage_) T(fn()); // fn may release, but will reacquire,
                                          // the GIL.
                is_initialized_ = true;   // This write is guarded by the GIL.
            });
            // All threads will observe `is_initialized_` as true here.
        }
        // Intentionally not returning `T &` to ensure the calling code is
        // self-documenting.
        return *this;
    }
