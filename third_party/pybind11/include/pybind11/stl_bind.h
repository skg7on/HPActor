...
template <typename T> struct container_traits {
    ...
    test_pair(typename T2::first_type*, typename T2::second_type*);
...
template <> struct is_comparable<recursive_bottom> : std::true_type {};
...
void vector_accessor(enable_if_t<!vector_needs_copy<Vector>::value, Class_>& cl) {
    ...
    cl.def(
        "__iter__",
        [](Vector& v) {
            return make_iterator<return_value_policy::reference_internal,
                                 ItType, ItType, T&>(v.begin(), v.end());
        },
...
struct vector_has_data_and_format<
    Vector, enable_if_t<std::is_same<decltype(format_descriptor<typename Vector::value_type>::format(),
...
class_<Vector, holder_type>
bind_vector(handle scope, std::string const& name, Args&&... args) {
...
template <typename Map, typename Class_>
void map_assignment(
    enable_if_t<!is_copy_assignable<typename Map::mapped_type>::value &&
    ...
    cl.def("__setitem__", [](Map& m, const KeyType& k, const MappedType& v) {
        ...
        if (!r.second) {
            // value type is not copy assignable so the only way to insert it is
            // to erase it first...
            m.erase(r.first);
            m.emplace(k, v);
        }
...
template <typename Map> struct KeysViewImpl : public detail::keys_view {
    explicit KeysViewImpl(Map& map) : map(map) {}
    size_t len() override {
        return map.size();
    }
    iterator iter() override {
        return make_key_iterator(map.begin(), map.end());
    }
...
template <typename Map> struct ValuesViewImpl : public detail::values_view {
...
template <typename Map> struct ItemsViewImpl : public detail::items_view {
    explicit ItemsViewImpl(Map& map) : map(map) {}
    size_t len() override {
        return map.size();
    }
    iterator iter() override {
        return make_iterator(map.begin(), map.end());
    }
    Map& map;
};
...
class_<Map, holder_type>
bind_map(handle scope, const std::string& name, Args&&... args) {
    using KeyType = typename Map::key_type;
    using MappedType = typename Map::mapped_type;
    ...
    if (!detail::get_type_info(typeid(KeysView))) {
        ...
        keys_view.def("__iter__", &KeysView::iter, keep_alive<0, 1>() /* Essential:
                                                                         keep
                                                                         view
                                                                         alive
                                                                         while
    ...
    if (!detail::get_type_info(typeid(ValuesView))) {
        ...
        values_view.def("__iter__", &ValuesView::iter, keep_alive<0, 1>() /* Essential:
                                                                             keep
                                                                             view
                                                                             alive
                                                                             while
    ...
    if (!detail::get_type_info(typeid(ItemsView))) {
        ...
        items_view.def("__iter__", &ItemsView::iter, keep_alive<0, 1>() /* Essential:
                                                                           keep
                                                                           view
                                                                           alive
                                                                           while
