...
template <typename type> class duration_caster {
  public:
    ...
    static handle cast(const type& src, return_value_policy /* policy */,
        ...
        // Declare these special duration types so the conversions happen with
        // the correct primitive types (int)
...
class type_caster<std::chrono::time_point<std::chrono::system_clock, Duration>> {
  public:
    ...
    bool load(handle src, bool) {
        ...
        value = time_point_cast<Duration>(
            system_clock::from_time_t(std::mktime(&cal)) + msecs);
        return true;
    }
    ...
    cast(const std::chrono::time_point<std::chrono::system_clock, Duration>& src,
        ...
        // Get out microseconds, and make sure they are positive, to avoid bug
        // in eastern hemisphere time zones (cfr.
        ...
        std::time_t tt = system_clock::to_time_t(
            time_point_cast<system_clock::duration>(src - us));
